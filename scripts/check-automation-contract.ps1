...
$qolAutomations = @($config.automations | Where-Object { $null -ne $_ })
$supportAutomations = @($config.supportAutomations | Where-Object { $null -ne $_ })
...
$targetFragments = @{}
foreach ($automation in $trackedAutomations) {
    $expectedAutomationConfig[$automation.id] = @{
        Name = $automation.name
        Status = $automation.status
        RRule = $automation.rrule
    }
    if (@($automation.requiredPromptFragments).Count -gt 0) {
        $automationRequiredPromptFragments[$automation.id] = @($automation.requiredPromptFragments)
    }
    $targetFragments[$automation.id] = @($automation.targetFragments)
}
...
$allowedScaleSignals = @($config.allowedScaleSignals)
$allowedManifestStatuses = @($config.allowedManifestStatuses)
...
if ($automationIds.Count -eq 0) {
    $failures.Add("Automation config must define at least one automation.")
}

$duplicateAutomationIds = @($automationIds | Group-Object | Where-Object { $_.Count -gt 1 } | ForEach-Object { $_.Name })
foreach ($id in $duplicateAutomationIds) {
...
if ($requiredPromptFragments.Count -eq 0) {
    $failures.Add("Automation config must define requiredPromptFragments.")
}

if ($strictManifestCutoff -lt $telemetryCutoff) {
    $failures.Add("Automation config strictManifestCutoff must not be earlier than telemetryCutoff.")
}

foreach ($automation in $trackedAutomations) {
    foreach ($field in @("id", "name", "status", "rrule")) {
        if (-not $automation.$field) {
            $failures.Add("Automation config entry is missing field '$field'.")
        }
    }
    if (@($automation.targetFragments).Count -eq 0) {
        $failures.Add("Automation config entry $($automation.id) must define targetFragments.")
    }
}
...
foreach ($id in $automationIds) {
    ...
    $promptFragments = if ($automationRequiredPromptFragments.ContainsKey($id)) {
        $automationRequiredPromptFragments[$id]
    }
    else {
    ...
    if (Test-Path -LiteralPath $path) {
        try {
            ...
            if ($normalizedPath -like "*docs/progress/runs/*.json") {
                ...
                if ($timestamp -and $timestamp -ge $strictManifestCutoff) {
                    $timestampLocal = Get-JsonProperty -Object $jsonObject -Name "timestamp_local"
                    if (-not $timestampLocal) {
                        $failures.Add("Run manifest $relative must use canonical timestamp_local after $($strictManifestCutoff.ToString("o"))")
                    }
                    elseif (-not (Convert-ToManifestTimestamp -Value ([string]$timestampLocal))) {
                        $failures.Add("Run manifest $relative timestamp_local is not parseable")
                    }

                    $automationId = [string](Get-JsonProperty -Object $jsonObject -Name "automation_id")
                    if (-not $automationId) {
                        $failures.Add("Run manifest $relative is missing non-empty automation_id")
                    }

                    $status = [string](Get-JsonProperty -Object $jsonObject -Name "status")
                    if (-not $status -or $allowedManifestStatuses -notcontains $status) {
                        $failures.Add("Run manifest $relative status must be one of: $($allowedManifestStatuses -join ', ')")
                    }
                }
            }
        }
...
if (Test-Path -LiteralPath $manifestReadmePath) {
    ...
    foreach ($fragment in @("timestamp_local", "automation_id", "status", "accepted_mvp_pass", "duration_minutes", "output_kind", "automation_efficiency", "scale_signal")) {
...
if (Test-Path -LiteralPath $catalogPath) {
    ...
    $allMvpRequirementIds = 1..([int]$config.mvpRequirementCount) | ForEach-Object { "MVP-{0:D3}" -f $_ }
...
Write-Output "Automation contract check passed for $($qolAutomationIds.Count) QoL automations and $($supportAutomations.Count) support automations ($liveAutomationsValidated live, $snapshotFallbacks snapshot fallback)."
