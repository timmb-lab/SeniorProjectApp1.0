...
assertMatches("workspaceJs", /function adminConsoleSectionsForRoles\(roles\)[\s\S]*add\("adminPeople", "People"[\s\S]*add\("adminStudents", "Students"[\s\S]*add\("adminAssignments",[\s\S]*add\("programs", "Programs"[\s\S]*add\("adminImports", "Imports"[\s\S]*add\("adminReports", "Reports"[\s\S]*add\("audit", "Audit"/, "Admin Console nav must follow the operations IA order");
