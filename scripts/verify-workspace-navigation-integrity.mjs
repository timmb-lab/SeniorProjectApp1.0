...
assertMatches("workspaceJs", /renderScreenGuidance\(activeSection, primaryRole, roles, sections\)/, "Workspace app shell must render the shared screen guidance helper");
...
assertMatches("workspaceJs", /function screenActionImpactsFor\(sectionId = "overview"[\s\S]*Add proof[\s\S]*Save decision[\s\S]*Create or import[\s\S]*Audit filters narrow logged activity/, "Screen action-impact guide must explain safe navigation, saved changes, and audit filtering");
...
assertMatches("workspaceJs", /function screenVisibilityNotesFor\(sectionId = "overview"[\s\S]*Proof you add is visible to you and staff who are allowed to review or support that work[\s\S]*Setup passwords[\s\S]*Redacted rows/, "Screen visibility guide must explain student proof, account setup, and audit redaction");
...
assertMatches("workspaceJs", /function screenStartRequirementsFor\(sectionId = "overview"[\s\S]*Have the exact proof link or file ready before adding proof or sending work for review[\s\S]*Review proof and history before saving a Program Teacher decision[\s\S]*Set action, person, or record filters before investigating the log/, "Screen start guide must explain prerequisites for student proof, review decisions, and audit investigation");
