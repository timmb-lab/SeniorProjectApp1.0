...
function parseArgs(values = process.argv.slice(2)) {
  const parsed = {
    baseUrl: process.env.WORKSPACE_SMOKE_BASE_URL || "https://thecapstoneproject.com",
  };
  for (let index = 0; index < values.length; index += 1) {
    ...
    if (value === "--base-url") {
      ...
      console.log("Usage: node scripts/prove-sales-demo-hosted.mjs [--base-url https://thecapstoneproject.com]");
      process.exit(0);
    } else {
...
async function runHostedSalesDemoProof(args = {}, options = {}) {
  ...
  const baseUrl = new URL(args.baseUrl || "https://thecapstoneproject.com").origin;
