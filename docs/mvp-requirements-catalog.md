...
| MVP-010 | Track student progress records and status history from trusted server/database state. | `student-workflow-evidence` | D1 tables; Figma node `61:2`; alpha state transitions; `/api/submissions/:id/submit` and `/api/reviews/:submissionId/decision` update submissions, progress records, status history, and audit events. | Add stale-write handling and broader route tests; dashboard counts derive from DB/server state. | `foundation started` |
...
| MVP-013 | Validate evidence metadata and support private evidence links/uploads through Google Drive repository path. | `student-workflow-evidence` | D1 `evidence_artifacts`, Drive root/index artifacts, alpha evidence URL validation, health readiness fields for evidence root/index and Google Drive credential posture. | Drive upload credential/OAuth path exists; metadata links to submissions; invalid URLs/provider failure/access denied states tested. | `foundation started` |
...
| MVP-015 | Provide program teacher review queue, detail, comments, revision request, approval, status history, and dashboard count updates. | `staff-review-mentor` | Alpha teacher review actions; Figma nodes `31:2`, `61:2`; `/api/reviews/:submissionId/decision` persists approval/revision decisions with teacher/admin scope checks. | End-to-end student -> teacher -> revision -> resubmission -> approval loop works without code edits and persists history. | `alpha active` |
