...
| 24 | Medium | E | teacher | Detail tab depth | Detail panel could surface clearer latest feedback/next action. | Staff need intervention context. | Add existing returned review/comment fields to the detail summary. | Staff-only notes guarded by existing detail API visibility. | S | none | yes | `workspace.js` latest feedback summary; `tests/workspace-app.test.mjs` render guard | Detail render test. |
