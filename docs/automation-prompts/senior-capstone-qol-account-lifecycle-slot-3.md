...
Automation memory: C:\Users\bryan\.codex\automations\senior-capstone-qol-account-lifecycle-slot-3\memory.md. Read it first. If it is missing, create it before repo work. At closeout, write a concise summary with the slice chosen, validation, blocker/next action, touched requirement IDs, and current run time.
