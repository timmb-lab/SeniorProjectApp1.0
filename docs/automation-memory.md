...
- Auth edge-state integration coverage now exists for `/api/auth/login`, `/api/auth/me`, and `/api/auth/logout`, including disabled login, reset-required login, invalid login, expired-session `/me`, disabled-account `/me`, reset-required `/me`, and logout revocation: `tests/auth-login.integration.test.mjs`.
