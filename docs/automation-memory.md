...
- Auth edge-state and reset-completion integration coverage now exists for `/api/auth/login`, `/api/auth/complete-reset`, `/api/auth/me`, and `/api/auth/logout`, including disabled login, reset-required login, invalid login, expired-session `/me`, disabled-account `/me`, reset-required `/me`, logout revocation, reset-required credential rotation, password-version increment, stale-session revocation, and new-password login: `tests/auth-login.integration.test.mjs`.
...
- The canonical `workspace.html` route now renders a password-reset completion panel when login or session state returns `password_reset_required`; it posts to `/api/auth/complete-reset` and source/VM tests cover the visible `data-auth-action="complete-reset"` marker. In-app browser tooling was not exposed during the 2026-05-20 22:40 PT pass, so reset-panel browser proof remains a follow-up.
...
- Remaining account lifecycle depth: invitation/import, admin reset initiation, active-user credential rotation, and hosted/browser reset-panel proof after deployment. Live Cloudflare verification requires replacing the invalid token.
...
5. Implement account provisioning/import, admin reset initiation, active-user credential rotation, and known-gaps QA.
