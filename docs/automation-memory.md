...
- Bryan reset the Senior Capstone automation setup on 2026-05-18, then rebuilt it again as ten QoL target groups. On 2026-05-18 17:30 PT, the live registry was split into thirty single-slot QoL automations, three per target, because one automation with multiple daily BYHOUR values was not reliably firing. After the May 18-19 overnight audit showed 53 of 54 runs were read-only or policy-blocked, burn-down mode became the live contract: ten Slot 1 QoL automations remain ACTIVE and twenty Slot 2/3 companions are PAUSED reserve capacity. All prior active/standby, daily prototype, weekly audit, seven-category, hourly, and 20x project automation TOMLs are superseded by the QoL set.
...
Real daily MVP goal: minimum 2 accepted MVP passes per calendar day, stretch 3 when unblocked, and at least 14 accepted MVP passes per week until the 100-pass target is met or recalibrated. Until the Day 7 alpha is accepted, the first two accepted passes each day should usually be app-flow implementation or alpha verification, not broad design polish or production account hardening. The current 10 active QoL starts are burn-down capacity, not a goal to count scheduled starts as accepted passes; the 20 paused reserve starts should reactivate only after Slot 1 runs can write, validate, commit, and push durable work. The source-framework seed QoL target group must review the prior seven days of committed run evidence and adjust only this project's next-week daily goal/allocation in `docs/master-plan.md`, `docs/automation-memory.md`, and `docs/mvp-requirements-catalog.md` when evidence requires it.
...
- `D-2026-05-18-032`: split each Senior Capstone QoL target into three app-managed single-slot automations after Bryan reported that multiple daily times inside one automation were not reliably firing. Keep the single-slot registry shape, but current burn-down mode pauses Slot 2 and Slot 3 under `D-2026-05-19-001`.
- `D-2026-05-19-001`: after the May 18-19 overnight audit found 53 of 54 runs blocked or only partially productive, keep only the ten Slot 1 QoL automations ACTIVE, pause the twenty Slot 2/3 companions as reserve, and require writable preflight before a run spends time on implementation.

Current QoL automation IDs:
- Source framework seed: ACTIVE `senior-capstone-qol-source-framework-seed-2` (`00:03`); PAUSED `senior-capstone-qol-source-framework-seed-slot-2` (`08:03`) and `senior-capstone-qol-source-framework-seed-slot-3` (`16:03`).
- Drive upload OAuth: ACTIVE `senior-capstone-qol-drive-upload-oauth-2` (`00:51`); PAUSED `senior-capstone-qol-drive-upload-oauth-slot-2` (`08:51`) and `senior-capstone-qol-drive-upload-oauth-slot-3` (`16:51`).
- Protected evidence tests: ACTIVE `senior-capstone-qol-protected-evidence-tests-2` (`01:39`); PAUSED `senior-capstone-qol-protected-evidence-tests-slot-2` (`09:39`) and `senior-capstone-qol-protected-evidence-tests-slot-3` (`17:39`).
- Teacher review endpoints: ACTIVE `senior-capstone-qol-teacher-review-endpoints-2` (`02:27`); PAUSED `senior-capstone-qol-teacher-review-endpoints-slot-2` (`10:27`) and `senior-capstone-qol-teacher-review-endpoints-slot-3` (`18:27`).
- Immutable review history: ACTIVE `senior-capstone-qol-immutable-review-history-2` (`03:15`); PAUSED `senior-capstone-qol-immutable-review-history-slot-2` (`11:15`) and `senior-capstone-qol-immutable-review-history-slot-3` (`19:15`).
- Mentor presentation flow: ACTIVE `senior-capstone-qol-mentor-presentation-flow-2` (`04:03`); PAUSED `senior-capstone-qol-mentor-presentation-flow-slot-2` (`12:03`) and `senior-capstone-qol-mentor-presentation-flow-slot-3` (`20:03`).
- Admin ops endpoints: ACTIVE `senior-capstone-qol-admin-ops-endpoints-2` (`04:51`); PAUSED `senior-capstone-qol-admin-ops-endpoints-slot-2` (`12:51`) and `senior-capstone-qol-admin-ops-endpoints-slot-3` (`20:51`).
- Announcements: ACTIVE `senior-capstone-qol-announcements-2` (`05:39`); PAUSED `senior-capstone-qol-announcements-slot-2` (`13:39`) and `senior-capstone-qol-announcements-slot-3` (`21:39`).
- Account lifecycle: ACTIVE `senior-capstone-qol-account-lifecycle-2` (`06:27`); PAUSED `senior-capstone-qol-account-lifecycle-slot-2` (`14:27`) and `senior-capstone-qol-account-lifecycle-slot-3` (`22:27`).
- Cloudflare verification: ACTIVE `senior-capstone-qol-cloudflare-verification-2` (`07:15`); PAUSED `senior-capstone-qol-cloudflare-verification-slot-2` (`15:15`) and `senior-capstone-qol-cloudflare-verification-slot-3` (`23:15`).
...
- Burn-down mode is now the live contract: the ten Slot 1 QoL automations stay ACTIVE at `00:03` through `07:15` PT, and the twenty Slot 2/3 companions are PAUSED reserve capacity.
- Updated `scripts/automation-config.json`, `scripts/check-automation-contract.ps1`, `scripts/measure-automation-efficiency.ps1`, cadence/runbook/self-improvement docs, master plan, decision log, audit log, run log, and this memory so future checks enforce 10 active starts/day with the single-slot reserve registry intact.
- Do not reactivate Slot 2 or Slot 3 until a live audit shows Slot 1 can write, validate, commit, and push durable changes or exact committed blockers.
