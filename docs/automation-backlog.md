...
- `evidence`: Bryan needs a full-fledged alpha by Day 7. For planning, Day 1 is Monday, 2026-05-18 and the alpha gate is end of day Sunday, 2026-05-24 PT. Production user accounts may remain incomplete, but the whole app flow must work through seeded/demo personas or a clearly labeled role switcher. First alpha shell now exists at `alpha.html` with `/api/alpha/state`, static seed fallback, student dashboard/proposal/evidence metadata, teacher review actions, mentor meeting/presentation slot actions, admin/misc-admin views, audit timeline updates, route/persona scope, and initial runbook `docs/alpha-runbook.md`.
- `next action`: Persist alpha proposal/review/evidence/audit transitions through D1 or a server-owned demo-state API, add smoke tests/CI, verify mobile no-overflow, and attempt a Cloudflare preview deployment.
