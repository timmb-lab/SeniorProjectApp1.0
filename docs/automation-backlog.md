...
- `next action`: Set `CLOUDFLARE_API_TOKEN` so Wrangler can run remotely in non-interactive mode, then run `powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File .\\scripts\\verify-framework-seed-d1.ps1 -Remote` to apply/verify remote D1 migrations; finally re-run the admin test-account seed endpoint in preview/production to migrate alpha fixtures onto seeded requirement IDs (starting with `req-proposal-draft`).
...
- `evidence`: The PDFs require mentor meeting attendance/make-up tracking, outline approval, presentation time scheduling, presentation check-out/check-in, celebration setup photo, ingredient lists for food, and May 5 personal archive/export. Figma now specifies archive/export controls in active file `z4t4tFPAKrMDh6pIYOeEw6`, node `69:2`; meeting/presentation scheduling in node `78:2`; celebration/archive readiness in node `144:2`; and archive provider/retention states in node `149:2`. Meeting persistence exists in `migrations/0004_mentor_meetings.sql`, `/api/mentor/meetings`, and `tests/mentor-meetings.integration.test.mjs`. Presentation slot persistence now exists in `migrations/0006_presentation_slots.sql`, `/api/presentation-slots`, `/api/presentation-slots/:id/check-out`, `/api/presentation-slots/:id/check-in`, and `tests/presentation-slots.integration.test.mjs`, with scoped visibility, admin/program-teacher scheduling, same-location conflict blocking, day-of check-out/check-in status/timestamps, and audit events for denied/conflict/scheduled/transition outcomes. The canonical workspace consumes `/api/presentation-slots` and renders presentation schedule, outline status, empty state, and staff-only check-out/check-in controls. Archive readiness now exists in `/api/student/archive/readiness`, deriving Celebration Day evidence, ingredient-list, thank-you/mentor note, reflection/portfolio, archive export, provider/signed-download readiness, retention policy/window state, and scoped manifest readiness from persisted progress/evidence/export/artifact rows with view/denial audit events and storage IDs redacted. Migration `0007_archive_export_artifacts.sql`, `/api/admin/exports/student-archive`, `/api/exports/:id/download`, and `functions/_lib/archive-export.ts` now verify Drive provider readiness before successful archive generation, record failed provider states as failed exports with audit events, generate/store/hash/expire/stream scoped JSON archive manifests only when provider checks pass, and expose configurable retention metadata. The canonical workspace now renders a student Archive tab for the May 5 package checklist, a manifest download link when available, and retention-policy state; focused archive/source/workspace tests passed on 2026-05-20 21:38 PT.
- `next action`: Apply/verify `0006_presentation_slots.sql` and `0007_archive_export_artifacts.sql` remotely when `CLOUDFLARE_API_TOKEN` is available, persist outline approval gates beyond slot status, add hosted presentation/archive dashboard proof after deployment, then add Drive-backed package files or signed-link delivery after Drive credentials are available.
