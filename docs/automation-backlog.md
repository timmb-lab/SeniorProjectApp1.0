...
- `next action`: Remote migrations `0006` and `0007` are now applied/recorded. Persist outline approval gates beyond slot status, fix Drive service-account root/index sharing, add hosted presentation/archive dashboard proof after Drive access passes, decide whether signed Drive links are needed beyond scoped app downloads, and confirm retention policy.
- `last updated`: 2026-05-20 23:11 PT
