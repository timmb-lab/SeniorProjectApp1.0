...
- `evidence`: The PDFs require mentor meeting attendance/make-up tracking, outline approval, presentation time scheduling, presentation check-out/check-in, celebration setup photo, ingredient lists for food, and May 5 personal archive/export. Figma now specifies the archive/export control surface in active file `z4t4tFPAKrMDh6pIYOeEw6`, node `69:2`, including scoped export request, signed archive download, student own-archive access, denied-access audit events, and retention-policy cues, but none of the meeting/presentation/celebration/archive workflows are implemented yet.
- `next action`: Create implementation slices after SC-001/SC-003 for meeting attendance, presentation slots, celebration evidence, and archive/export workflow; consume Figma node `69:2` when implementing `StudentArchiveExport`, `ExportRequest`, `ExportArtifact`, signed downloads, and archive/export audit events.
...
- `evidence`: `docs/master-plan.md` now defines the revised MVP as a secure database-backed app with users, groups, roles, progress updates, private evidence, audit logs, dashboards, announcements, exports, and GitHub-to-Cloudflare hosting. `docs/architecture/adr-0001-stack-auth-database-upload.md` is accepted as the default Cloudflare-compatible stack path. Active Figma node `48:2` specifies the admin provisioning states, routes, data records, permissions, audit events, duplicate import handling, and narrow misc-admin scope needed for the users/groups/roles portion of this scaffold. Active Figma node `61:2` specifies progress-update, status-history, dashboard-aggregate, stale-conflict, permission, and audit-first acceptance checks for the first admin/progress vertical slice. Active Figma node `69:2` specifies immutable audit-event filters, redacted sensitive event detail, scoped export request, signed archive downloads, permission denial, retention-policy cues, and export/download/view audit checks. No production app scaffold, managed auth, database, private file storage, migrations, API layer, tests, CI, or GitHub-to-Cloudflare deployment pipeline exists yet.
- `next action`: Rebuild lane must scaffold the selected stack with auth/database/user-group/progress/private-upload/audit/export/deployment assumptions explicit, consuming Figma nodes `18:2`, `37:2`, `43:2`, `48:2`, `56:2`, `61:2`, and `69:2` for route/data/permission contracts.
- `last updated`: 2026-05-18 11:03 PT
