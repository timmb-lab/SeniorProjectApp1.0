...
- `next action`: Apply/verify `0006_presentation_slots.sql` and `0007_archive_export_artifacts.sql` remotely when `CLOUDFLARE_API_TOKEN` is available, persist outline approval gates beyond slot status, add hosted presentation/archive dashboard proof after deployment and Drive secrets, decide whether signed Drive links are needed beyond scoped app downloads, and confirm retention policy.
...
- `next action`: Deepen the alpha into real workflow endpoints and add the known-gaps file, broader permission tests, mobile/error/empty QA, invitation/import/admin reset initiation, and Drive upload credential/OAuth implementation.
