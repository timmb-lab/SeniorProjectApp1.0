...
- `next action`: Rebuild should treat `/api/student/dashboard` route-level access auditing, `/api/teacher/review-queue` empty-scope/audit hardening, `/api/reviews/:submissionId/history` access auditing, `/api/reviews/:submissionId/decision` unauthorized/denied audit hardening, `/api/submissions/:id/evidence` evidence-link access auditing, and `/api/submissions/:id/submit` submit access auditing as consumed, then extend the same role/scope/audit matrix to remaining workflow endpoints: submission detail/readback, evidence upload/download browser/live proof, archive/export, mentor meetings, presentation slots, and any dashboard aggregate readbacks. It should also capture hosted browser proof for no-assignment and section-level permission-denied states with fake `.test` accounts only.
...
- `latest Figma handoff`: 2026-05-21 node `187:2` was added to mark `/api/submissions/:id/submit` submit access audit proof consumed after the route gained missing-session, denied, missing-evidence blocked, and successful submit audits. Next protected-record rebuild work should cover submission detail/readback, evidence upload/download proof, mentor-meeting, presentation-slot, archive/export endpoints, and hosted no-assignment plus section-level permission-denied UI proof. Node `151:2` remains the Drive archive delivery contract; it is partially consumed by the repo-side Drive package upload path, with hosted fake-account proof still blocked by Drive upload HTTP 403.
- `last updated`: 2026-05-21 05:30 PT
