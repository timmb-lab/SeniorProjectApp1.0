...
- `artifact`: active Figma file `z4t4tFPAKrMDh6pIYOeEw6`, `03 Product Preview + State Variants` node `6:2`, `100-Pass MVP Execution Map` node `18:2`, route/data contract nodes `18:44` through `18:47`, review/override variants node `31:2`, developer handoff contract node `31:144`, private evidence/review-history contract node `37:2`, evidence handoff contract node `37:177`, MVP component variant matrix node `43:2`, status component set `43:55`, action component set `43:73`, evidence row component set `43:149`, shared UI contract `43:150`, admin account/group provisioning contract node `48:2`, admin provisioning handoff node `48:208`, mobile evidence/revision contract node `56:2`, mobile handoff node `56:114`
- `next action`: Consume the route/data/permission contract while scaffolding the accepted Cloudflare database/auth/progress foundation, then implement shared `StatusPill`, `ActionButton`, `EvidenceArtifactRow`, `PermissionGate`, and `ReviewHistoryItem` primitives before wiring teacher review decisions, admin override paths, private evidence upload/link access checks, immutable review history, admin user/group/role/cohort provisioning, and mobile student evidence/revision states from nodes `31:2`, `37:2`, `43:2`, `48:2`, and `56:2`.
- `acceptance check`: Rebuild commits a stack/scaffold or implementation plan that maps `/student/progress`, `/teacher/review`, `/mentor/assigned`, `/admin/users`, `/admin/groups`, `/admin/programs`, `/admin/cohorts`, `/admin/audit`, `/student/evidence`, `/student/submissions/:submissionId/revise`, `/api/submissions/:id/evidence`, `/api/evidence/:id/check-access`, `/api/reviews/:id/history`, `/api/admin/users/import`, `/api/admin/role-assignments`, `/api/admin/mentor-assignments`, review decisions, admin overrides, evidence upload/link checks, review history, shared component variants, admin provisioning states, and mobile revision/blocked-submit/access-denied states to persisted data, server authorization, audit events, private-evidence guards, signed URL expiry, stable loading/disabled states, accessible text-plus-color status, duplicate import handling, narrow misc-admin scope, mobile no-overflow behavior, and loading/error/permission tests.
- `evidence to close`: Committed rebuild plan or code scaffold referencing `z4t4tFPAKrMDh6pIYOeEw6`, node `18:2`, node `31:2`, node `37:2`, node `43:2`, node `48:2`, and node `56:2`.
