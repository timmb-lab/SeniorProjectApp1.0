...
- `next action`: Finish the scaffold by adding tests/CI, first-admin bootstrap, server-side Drive upload credentials/OAuth, D1/server-owned persistence for alpha proposal/review/evidence/audit transitions, and first admin/progress workflow endpoints.
...
- `next action`: Continue consuming the route/data/permission contract by persisting the new alpha route/persona shell into D1/server-owned workflow APIs, then implement shared `StatusPill`, `ActionButton`, `EvidenceArtifactRow`, `PermissionGate`, and `ReviewHistoryItem` primitives before hardening teacher review decisions, admin override paths, private evidence upload/link access checks, immutable review history, admin user/group/role/cohort provisioning, mobile student evidence/revision states, progress-update/dashboard-aggregate persistence, immutable audit-log filters, export-request authorization, signed archive downloads, meeting attendance, make-up linkage, outline approval gates, presentation slot conflict checks, and audited check-out/check-in states from nodes `31:2`, `37:2`, `43:2`, `48:2`, `56:2`, `61:2`, `69:2`, and `78:2`.
...
- `evidence to close`: Committed rebuild plan or code scaffold referencing `z4t4tFPAKrMDh6pIYOeEw6`, node `18:2`, node `31:2`, node `37:2`, node `43:2`, node `48:2`, node `56:2`, node `61:2`, node `69:2`, and node `78:2`. First alpha shell evidence now exists in `alpha.html`, `alpha.js`, `alpha.css`, `/api/alpha/state`, and `docs/alpha-runbook.md`; persistence/tests remain before close.
- `last updated`: 2026-05-18 11:45 PT
