...
- `next action`: Admin import UI, hosted source-marker proof, and reset-first API proof now exist. Node `163:2` now marks API proof consumed and narrows the remaining work to browser-level fake `.test` proof for validation UI, no-store setup output clearing after refresh, reset-first login before dashboards, denied-role UI behavior, refresh/stale-session safety, and no credential leakage. After that, get a Bryan-approved delivery policy before real pilot users receive credentials.
...
- `evidence to close`: Partial consumption landed on 2026-05-21: `workspace.js` renders an admin-only Users tab with `data-admin-action="import-users"`, posts to `/api/admin/users/import`, keeps the response marked as no-store, and renders one-time setup passwords only from the immediate response; `tests/workspace-app.test.mjs` covers the panel and returned setup output; hosted `workspace.js` marker proof passed on poll 2 after implementation commit `5b23d15`. The 01:37 PT non-Figma pass added `admin_users_import_denied` audit coverage and an imported-account reset-first API proof in `tests/admin-users-import.integration.test.mjs`, including no-store response, `password_reset_required` login, reset completion, post-reset sign-in, role readback, and no temporary/replacement password leakage in audit metadata. The 01:49 PT Figma pass updated node `163:2` to mark those API checks consumed and passed `use_figma`, `get_design_context`, and `get_screenshot` verification. Remaining evidence: browser-level fake-account proof for validation errors, reset-required first login, permission-denied role attempts, refresh/stale-session safety, and accepted decision `HD-2026-05-21-001` or an implemented invitation/email delivery alternative.
- `last updated`: 2026-05-21 01:49 PT
...
- `next action`: API proof for import/reset is now consumed and recorded in node `163:2`. Consume the remaining credential lifecycle proof needs by adding browser-level import/reset proof, reset-panel proof, signed-in password-change proof, admin reset-required initiation proof, stale-session fallback proof, temporary credential display policy, and redacted audit coverage.
