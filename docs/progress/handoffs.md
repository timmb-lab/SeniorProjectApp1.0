...
- `next action`: Implement celebration evidence, ingredient-list requirement, thank-you/mentor note, reflection/portfolio completion, May 5 archive package, scoped signed download, expired download, provider-unavailable, and archive permission-denied states from persisted rows and audit events.
- `acceptance check`: Canonical workspace or admin/export UI derives archive readiness from `EvidenceArtifact`, `RequirementProgress`, `Review`, `StudentArchiveExport`, `ExportRequest`, `ExportArtifact`, and `AuditEvent` rows; students can access only their own archive; admin export requires reason/scope; misc-admin access is explicitly scoped; expired/download/denied/provider-unavailable paths are audited and never expose Drive storage IDs.
- `evidence to close`: Rebuild commit with focused tests for student own archive, out-of-scope denial, admin reason/scope, misc-admin scope, expired signed download, provider unavailable, and audit events; run log and manifest referencing Figma node `144:2`.
- `last updated`: 2026-05-20 20:18 PT
...
- `latest Figma handoff`: 2026-05-20 node `144:2` adds celebration evidence, thank-you/mentor note, reflection/portfolio, archive request, expired download, provider-unavailable, and archive permission-denied states; node `139:2` was already consumed by the non-Figma workspace Presentation tab for persisted slot visibility and staff check-out/check-in controls.
- `last updated`: 2026-05-20 20:18 PT
