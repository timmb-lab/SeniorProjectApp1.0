...
- `next action`: Rebuild should treat `/api/student/dashboard` route-level access auditing, `/api/teacher/review-queue` empty-scope/audit hardening, `/api/reviews/:submissionId/history` access auditing, and `/api/reviews/:submissionId/decision` unauthorized/denied audit hardening as consumed, then extend the same role/scope/audit matrix to remaining workflow endpoints: submission submit/detail, evidence access/download/upload, archive/export, mentor meetings, presentation slots, and any dashboard aggregate readbacks. It should also capture hosted browser proof for no-assignment and section-level permission-denied states with fake `.test` accounts only.
