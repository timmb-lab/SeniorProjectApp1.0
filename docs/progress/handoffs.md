...
- `next action`: Broaden no-hidden-core-content coverage proof across every public guide route, then complete live Drive upload/download once Bryan adds the missing Google Drive credential secrets. Consume Figma node `133:2` when expanding canonical workspace UI coverage for disabled/reset-required/no-assignment, session-expired, and live section-level permission-denied states; role-pending and source-level permission-denied rendering now have local proof.
- `acceptance check`: Public website mode toggle stays visible with source-aligned Student/Teacher content; normal public nav stays free of alpha/account/internal QA links; canonical `workspace.html` route requires auth and chooses screens from D1-backed role/scope, with credential-backed proof for local student login/session/evidence/upload-block/logout, local role routes, no-role role-pending UI, and source-executed permission-denied UI. Live Drive upload/download remains required before pilot uploads.
- `evidence to close`: Passing `check:production-surfaces`, `check:generated-output-drift`, `check`, `tests/workspace-app.test.mjs`, credential-backed `tests/workspace-browser-smoke.test.mjs` against local Pages dev, in-app browser signed-in workspace/evidence/logout proof, in-app browser no-role role-pending proof, direct live signed-out/signed-in workspace proof, disabled/reset/no-assignment UI proof, and live Drive upload/download after Cloudflare Drive secrets are configured.
...
- `latest Figma handoff`: 2026-05-20 node `133:2` now distinguishes workspace account lifecycle and assignment/scope edge states for session expired, disabled account, reset required, role pending, no active assignment, and section permission denied, with route/data/audit guardrails for canonical `/workspace` implementation.
- `last updated`: 2026-05-20 17:37 PT
