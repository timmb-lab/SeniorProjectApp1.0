...
- `next action`: Rebuild should treat `/api/student/dashboard`, `/api/teacher/review-queue`, `/api/reviews/:submissionId/history`, `/api/reviews/:submissionId/decision`, `/api/submissions/:id/evidence`, `/api/submissions/:id/submit`, `/api/submissions/:id`, `/api/mentor/meetings`, presentation slots, archive/export denial paths, hosted evidence upload/download proof, and the new admin/program-teacher/mentor dashboard routes as consumed for route-level access auditing. Use node `196:2` as the checklist for hosted permission-state proof, dashboard aggregate readbacks, hosted presentation/archive UI proof, tenant/SSO denied-state proof, and any remaining workflow family using fake `.test` accounts only.
...
- `evidence to close`: Passing hosted browser/API evidence for no-assignment and section-level permission-denied UI. Presentation slot proof now exists in `/api/presentation-slots`, `/api/presentation-slots/:id/check-out`, `/api/presentation-slots/:id/check-in`, and `tests/presentation-slots.integration.test.mjs`, covering missing-session audits, student-own/student-other no-leak list behavior, misc-admin denial, empty-scope teacher no-leak behavior, scoped viewed counts, scheduling/conflict checks, and check-out/check-in unauthorized/denied/success audits. Archive/export proof now exists in `/api/student/archive/readiness`, `/api/admin/exports/student-archive`, `/api/exports/:id/download`, and `tests/archive-readiness.integration.test.mjs`, covering missing-session readiness/export/download audits, non-admin export denial, provider/package failures, scoped downloads, missing artifacts, expiry, and no raw Drive ID exposure. Hosted evidence proof exists in `scripts/check-google-drive-live.mjs`, `/api/student/dashboard`, `/api/evidence/:id/download`, `workspace.js`, and tests, covering safe dashboard download URLs, hosted workspace download markers, fake `.test` text upload/download, >5MB resumable upload/download, D1 upload/download audits, denial guards, and storage-ID leak checks. The 2026-05-21 tenant dashboard pass added `/api/admin/dashboard`, `/api/program-teacher/dashboard`, `/api/mentor/dashboard`, role-context helpers, scoped route tests, audit writes, no-storage-secret response assertions, and workspace command-center renderers that consume node `196:2` for protected-record dashboard acceptance. Figma verification for node `196:2` remains the acceptance-board reference.
- `last updated`: 2026-05-21 15:48 PT
...
- `evidence to close`: Partial consumption landed on 2026-05-21: `workspace.js` renders an admin-only Users tab with `data-admin-action="import-users"`, posts to `/api/admin/users/import`, keeps the response marked as no-store, and renders one-time setup passwords only from the immediate response; `tests/workspace-app.test.mjs` covers the panel and returned setup output; hosted `workspace.js` marker proof passed on poll 2 after implementation commit `5b23d15`. The 01:37 PT non-Figma pass added `admin_users_import_denied` audit coverage and an imported-account reset-first API proof in `tests/admin-users-import.integration.test.mjs`, including no-store response, `password_reset_required` login, reset completion, post-reset sign-in, role readback, and no temporary/replacement password leakage in audit metadata. The 02:10 PT non-Figma pass added local credential-backed Pages/D1 proof in `tests/workspace-browser-smoke.test.mjs`. The 08:18 PT pass added `admin_users_import_blocked` policy guard coverage, proving real non-`.test` admin-visible temporary credential imports are blocked by default and allowed only with explicit override. Remaining evidence: accepted decision `HD-2026-05-21-001` or an implemented invitation/email delivery alternative before real pilot users receive credentials.
- `last updated`: 2026-05-21 08:18 PT
...
- `latest Figma handoff`: 2026-05-21 node `196:2` was added to consolidate the protected-record audit chain after node `190:2` marked `GET /api/submissions/:id` submission detail/readback access audit proof consumed. The non-Figma lane has now applied that checklist to `/api/mentor/meetings` read/mutation access, hosted evidence upload/download proof, and the new admin/program-teacher/mentor dashboard route matrix. Node `61:2` is now consumed in production workspace code through D1-derived dashboard counts, status-derived KPI cards, scoped program breakdowns, admin needs-attention cards, mentor assignment summaries, and tests proving the dashboards do not use client-only source-of-truth data. Next protected-record rebuild work should use node `196:2` as the acceptance checklist for hosted no-assignment plus section-level permission-denied UI proof, tenant SSO denied-state proof, remaining archive-specific hosted proof, and future student-record drilldowns. Node `151:2` remains the Drive archive delivery contract; it is partially consumed by the repo-side Drive package upload path, with archive-specific hosted fake-account proof still open.
- `last updated`: 2026-05-21 15:48 PT
