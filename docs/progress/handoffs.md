...
- `next action`: Apply/verify migration `0007_archive_export_artifacts.sql` remotely when Cloudflare token access is available, then capture hosted fake-account proof for the Drive-backed archive package upload/app-scoped download path after Drive credentials are configured; decide whether a separate signed Drive-link delivery is needed beyond scoped app downloads.
...
- `evidence to close`: Hosted fake-account proof for Drive-backed package upload/app-scoped download markers plus remote migration readback. First consumption slice landed previously: `/api/student/archive/readiness`, workspace Archive tab, explicit admin archive reason, truthful signed-download disabled state, `tests/archive-readiness.integration.test.mjs`, workspace tests/smoke, and in-app browser student Archive tab proof. The 2026-05-20 21:10 PT non-Figma slice added `export_artifacts`, scoped JSON archive manifest generation, authenticated manifest download, expired-package retry, workspace manifest link markers, and focused integration/source/workspace tests. The 2026-05-20 21:17 PT Figma slice added node `149:2` with provider-unavailable, signed-link, remote migration, and retention-policy route/data/permission states. The 2026-05-20 21:38 PT non-Figma slice partially consumed node `149:2` by provider-gating archive generation, auditing failed provider states, adding configurable retention metadata, and rendering workspace retention state. The 2026-05-20 22:09 PT non-Figma slice partially consumed node `151:2` by uploading redacted manifest JSON into Drive after provider probes, storing Drive file IDs only server-side, and exposing only Drive-package-ready markers/headers.
- `last updated`: 2026-05-20 22:09 PT
...
- `next action`: Apply/verify remote migration `0007`, capture hosted archive UI/API proof after `CLOUDFLARE_API_TOKEN` plus Google Drive credential secrets are available, and decide whether signed Drive links are required beyond the scoped app download endpoint.
...
- `next action`: After `CLOUDFLARE_API_TOKEN` and Google Drive credential secrets are available, apply/read back remote migration `0007`, capture hosted fake-account proof for admin archive generation, Drive package upload markers, student archive download states, expired retry, and permission denial, then decide whether a separate signed Drive-link flow is needed.
...
- `latest Figma handoff`: 2026-05-20 node `151:2` added Drive-backed archive delivery and hosted-proof states. It is now partially consumed by the repo-side Drive package upload path; remaining implementation depth is remote migration readback, hosted fake-account proof after secrets, signed-link policy if required, and no raw Drive ID rendering in hosted package/download states. Node `139:2` was already consumed by the non-Figma workspace Presentation tab for persisted slot visibility and staff check-out/check-in controls.
- `last updated`: 2026-05-20 21:17 PT
