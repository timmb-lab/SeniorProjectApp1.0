...
- `next action`: Rebuild should treat `/api/student/dashboard`, `/api/teacher/review-queue`, `/api/reviews/:submissionId/history`, `/api/reviews/:submissionId/decision`, `/api/submissions/:id/evidence`, `/api/submissions/:id/submit`, `/api/submissions/:id`, `/api/mentor/meetings`, presentation slots, archive/export denial paths, and hosted evidence upload/download proof as consumed for route-level access auditing. Use node `196:2` as the checklist for hosted permission-state proof, dashboard aggregate readbacks, hosted presentation/archive UI proof, and any remaining workflow family using fake `.test` accounts only.
