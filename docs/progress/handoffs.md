...
- `next action`: Admin import UI and hosted source-marker proof now exist in the canonical workspace, and node `163:2` now defines the remaining proof sequence. API proof now covers duplicate/scope validation, no-store setup output, reset-required first login, denied-role audit behavior, and no credential leakage. Next, capture browser-level fake `.test` proof for the same import/reset states plus refresh/stale-session safety, then get a Bryan-approved delivery policy before real pilot users receive credentials.
...
- `evidence to close`: Partial consumption landed on 2026-05-21: `workspace.js` renders an admin-only Users tab with `data-admin-action="import-users"`, posts to `/api/admin/users/import`, keeps the response marked as no-store, and renders one-time setup passwords only from the immediate response; `tests/workspace-app.test.mjs` covers the panel and returned setup output; hosted `workspace.js` marker proof passed on poll 2 after implementation commit `5b23d15`. Figma node `163:2` now records the remaining proof states and passed `use_figma`, `get_design_context`, and `get_screenshot` verification. The 01:37 PT non-Figma pass added `admin_users_import_denied` audit coverage and an imported-account reset-first API proof in `tests/admin-users-import.integration.test.mjs`, including no-store response, `password_reset_required` login, reset completion, post-reset sign-in, role readback, and no temporary/replacement password leakage in audit metadata. Remaining evidence: browser-level fake-account proof for validation errors, reset-required first login, permission-denied role attempts, refresh/stale-session safety, and accepted decision `HD-2026-05-21-001` or an implemented invitation/email delivery alternative.
...
- `next action`: Consume the remaining node `153:2` and `158:2` proof needs by adding browser-level import/reset proof, reset-panel proof, signed-in password-change proof, admin reset-required initiation proof, stale-session fallback proof, temporary credential display policy, and redacted audit coverage.
