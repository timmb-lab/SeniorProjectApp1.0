...
- `owner lane`: non-figma / protected-evidence-tests, student-workflow-evidence, backend-security-data, and deployment-qa
- `status`: open
- `source`: MVP-028 protected-record access audit handoffs
- `artifact`: active Figma file `z4t4tFPAKrMDh6pIYOeEw6`, page `05 Full MVP Alpha Prototype`, node `173:2` (`Prototype / 24 / Student dashboard access audit handoff`), node `178:2` (`Prototype / 25 / Teacher review queue access audit handoff`), node `180:2` (`Prototype / 26 / Review history and decision access audit handoff`), node `183:2` (`Prototype / 27 / Evidence link access audit handoff`), node `187:2` (`Prototype / 28 / Submission submit access audit handoff`), node `190:2` (`Prototype / 29 / Submission detail access audit handoff`), node `196:2` (`Prototype / 30 / Protected record audit acceptance board`), shared plugin data keys `senior_capstone/student_dashboard_access_audit_contract_2026_05_21`, `senior_capstone/teacher_review_queue_scope_audit_contract_2026_05_21`, `senior_capstone/review_history_decision_access_audit_contract_2026_05_21`, `senior_capstone/evidence_link_access_audit_contract_2026_05_21`, `senior_capstone/submission_submit_access_audit_contract_2026_05_21`, `senior_capstone/submission_detail_access_audit_contract_2026_05_21`, and `senior_capstone/protected_record_acceptance_matrix_2026_05_21`, and repo notes in `docs/progress/figma.md`.
- `next action`: Rebuild should treat `/api/student/dashboard` route-level access auditing, `/api/teacher/review-queue` empty-scope/audit hardening, `/api/reviews/:submissionId/history` access auditing, `/api/reviews/:submissionId/decision` unauthorized/denied audit hardening, `/api/submissions/:id/evidence` evidence-link access auditing, `/api/submissions/:id/submit` submit access auditing, `/api/submissions/:id` submission-detail access auditing, `/api/mentor/meetings` scoped read/mutation access auditing, and hosted evidence upload/download proof as consumed, then use node `196:2` as the acceptance checklist for one remaining workflow endpoint family: archive/export, presentation slots, dashboard aggregate readbacks, or hosted no-assignment plus section-level permission-denied states with fake `.test` accounts only.
- `acceptance check`: Remaining endpoint tests prove student-own, student-other denial, active/inactive mentor assignment, program/cohort teacher scope, admin inspection, misc-admin broad denial, redacted audit metadata, no Drive storage identifier exposure, and production-safe 401/403 UI behavior.
- `evidence to close`: Passing route-level tests for the next protected workflow endpoint set plus hosted browser evidence for no-assignment and section-level permission-denied UI. The source dashboard proof exists in `tests/student-dashboard-access.integration.test.mjs`, route implementation `/api/student/dashboard`, and non-Figma run `2026-05-21-0306-student-dashboard-access-audit-mvp-006`; teacher review queue scope/audit proof exists in `/api/teacher/review-queue` and `tests/review-loop.integration.test.mjs`, covering missing-session audit, misc-admin denial, empty program-scope no-leak behavior, and reviewed queue audit metadata. Review history/decision protected-record proof exists in `/api/reviews/:submissionId/history`, `/api/reviews/:submissionId/decision`, and `tests/review-loop.integration.test.mjs`, covering history unauthorized/denied/viewed audits plus decision unauthorized/denied audits with role scopes. Evidence-link proof exists in `/api/submissions/:id/evidence` and `tests/evidence-link.integration.test.mjs`, covering missing-session audit, cross-student denial, non-student role denial, successful link metadata insert, actor role scopes, and no Drive storage-ID response leakage. Submit proof exists in `/api/submissions/:id/submit` and `tests/review-loop.integration.test.mjs`, covering missing-session audit, cross-student denial, staff denial for mentor/program-teacher/admin/misc-admin, missing-evidence blocked submit audit, successful submit audit metadata, role scopes, and immutable version snapshot preservation. Submission-detail proof exists in `/api/submissions/:id` and `tests/review-loop.integration.test.mjs`, covering missing-session audit, student-other and misc-admin denial, student-own/assigned mentor/program-teacher/admin readback, actor role scopes, and no Drive storage identifier exposure. Mentor-meeting proof exists in `/api/mentor/meetings` and `tests/mentor-meetings.integration.test.mjs`, covering missing-session read/write audits, misc-admin read denial, student-own readback, student-other denial, active assigned mentor view, inactive mentor denial, scoped program-teacher view, empty-scope teacher denial, admin inspection, mentor-only writes, assignment-scope denial, held-meeting audit metadata, and actor role scopes. Hosted evidence proof now exists in `scripts/check-google-drive-live.mjs`, `/api/student/dashboard`, `/api/evidence/:id/download`, `workspace.js`, and tests, covering safe dashboard download URLs, hosted workspace download markers, fake `.test` text upload/download, >5MB resumable upload/download, D1 upload/download audits, denial guards, and storage-ID leak checks. Figma verification for node `173:2` passed with `get_design_context`, `get_screenshot`, 49 text nodes, zero suspicious clipped text nodes, and zero child overflow. Figma verification for node `178:2` passed with `get_design_context`, `get_screenshot`, 49 text nodes, zero suspicious clipped text nodes, zero direct-child overflow, zero collapsed frames, and screenshot `1010x1024` from original `1360x1379`. Figma verification for node `180:2` passed with `get_design_context`, `get_screenshot`, 50 text nodes, zero suspicious clipped text nodes, zero child overflow, and screenshot `952x1024` from original `1360x1463`. Figma verification for node `183:2` passed with `get_design_context`, `get_screenshot`, 50 text nodes, zero suspicious clipped text nodes, zero child overflow, and screenshot `972x1024` from original `1360x1434`. Figma verification for node `187:2` passed with `get_design_context`, `get_screenshot`, 50 text nodes, zero suspicious clipped text nodes, zero child overflow, and screenshot `1024x1013` from original `1360x1345`. Figma verification for node `190:2` passed with `get_design_context`, `get_screenshot`, 51 text nodes, zero collapsed text nodes, zero suspicious clipped text nodes, zero child overflow, and screenshot `1009x1024` from original `1360x1381`. Figma verification for acceptance board node `196:2` passed with `get_design_context`, `get_screenshot`, 122 text nodes, zero suspicious clipped text nodes, zero child overflow, consumed-node references present, remaining proof queue present, and screenshot `512x1024` from original `1360x2724`.
- `last updated`: 2026-05-21 07:46 PT
...
- `artifact`: active Figma file `z4t4tFPAKrMDh6pIYOeEw6`, page `05 Full MVP Alpha Prototype`, node `158:2` (`Prototype / 22 / Admin import temporary credential handoff`), node `163:2` (`Prototype / 23 / Admin import proof QA handoff`), shared plugin data keys `senior_capstone/admin_import_temp_credential_contract_2026_05_21` and `senior_capstone/admin_import_proof_qa_contract_2026_05_21`, and repo notes in `docs/progress/figma.md`.
- `next action`: Admin import UI, hosted source-marker proof, reset-first API proof, and local credential-backed Pages/D1 fake `.test` proof now exist. Remaining work is Bryan-approved real-user credential delivery before pilot imports, optional hosted visual proof after deploy, and broader role-scope/protected-record hardening.
...
- `evidence to close`: Partial consumption landed on 2026-05-21: `workspace.js` renders an admin-only Users tab with `data-admin-action="import-users"`, posts to `/api/admin/users/import`, keeps the response marked as no-store, and renders one-time setup passwords only from the immediate response; `tests/workspace-app.test.mjs` covers the panel and returned setup output; hosted `workspace.js` marker proof passed on poll 2 after implementation commit `5b23d15`. The 01:37 PT non-Figma pass added `admin_users_import_denied` audit coverage and an imported-account reset-first API proof in `tests/admin-users-import.integration.test.mjs`, including no-store response, `password_reset_required` login, reset completion, post-reset sign-in, role readback, and no temporary/replacement password leakage in audit metadata. The 01:49 PT Figma pass updated node `163:2` to mark those API checks consumed and passed `use_figma`, `get_design_context`, and `get_screenshot` verification. The 02:10 PT non-Figma pass added local credential-backed Pages/D1 proof in `tests/workspace-browser-smoke.test.mjs` for admin validation failure, misc-admin denied import, no-store setup output, reset-required first login without session, reset completion, old setup credential rejection, replacement-password login, imported-student role readback, and redacted audit readback. `tests/workspace-app.test.mjs` also proves setup output clears when in-memory import state is gone and non-admin users see a permission-denied import state. Remaining evidence: accepted decision `HD-2026-05-21-001` or an implemented invitation/email delivery alternative before real pilot users receive credentials.
- `last updated`: 2026-05-21 02:10 PT
...
- `artifact`: active Figma file `z4t4tFPAKrMDh6pIYOeEw6`, page `05 Full MVP Alpha Prototype`, node `153:2` (`Prototype / 21 / Credential lifecycle handoff`), deeper import node `158:2` (`Prototype / 22 / Admin import temporary credential handoff`), proof QA node `163:2` (`Prototype / 23 / Admin import proof QA handoff`), shared plugin data keys `senior_capstone/credential_lifecycle_contract_2026_05_21`, `senior_capstone/admin_import_temp_credential_contract_2026_05_21`, and `senior_capstone/admin_import_proof_qa_contract_2026_05_21`, and repo notes in `docs/progress/figma.md`.
- `next action`: API proof and local credential-backed import/reset proof are now consumed for node `163:2`. Remaining credential lifecycle work is the real-user temporary credential delivery policy, optional hosted visual proof after deploy, broader role-scope hardening, and any deeper active-user/admin-reset visual proof needed before pilot.
...
- `evidence to close`: `/api/admin/users/import` now covers admin-only import, generated reset-required temporary credentials, initial role assignment, duplicate/scope validation, authenticated non-admin denial audit, reset-first login enforcement, post-reset sign-in, and redacted audit metadata with `tests/admin-users-import.integration.test.mjs`; Figma node `158:2` named the hosted UI, one-time display, and delivery-policy states; the 2026-05-21 non-Figma pass added the admin-only workspace import panel, one-time setup password response rendering, source/VM coverage, and hosted marker proof. The 01:49 PT Figma update marks import/reset API proof consumed in node `163:2`. The 02:10 PT non-Figma pass added local Pages/D1 browser-smoke proof for import validation, denied role, no-store setup output, reset-first login, reset completion, old setup credential rejection, new-password login, role readback, and redacted audit checks; no passwords, temporary credentials, or real student records appear in docs, Figma, screenshots, logs, or chat. Remaining evidence: real-user delivery policy, optional hosted visual proof, broader active-user/admin-reset visual proof, and role-scope/protected-record hardening.
- `last updated`: 2026-05-21 02:10 PT
...
- `next action`: Closed for file-byte evidence live readiness. Continue browser-level upload progress/retry, Google Docs export cases, archive-specific hosted proof, and protected permission-state UI proof under the remaining handoffs.
- `acceptance check`: Satisfied. Drive live check passes with fake `.test` login, token exchange, root folder MIME proof, index sheet MIME proof, fake upload success, app-scoped download byte comparison, >5MB resumable upload/download proof, remote D1 evidence upload/download audit proof without selecting raw Drive IDs, and browser/API leakage checks.
- `evidence to close`: `npm run check:drive:live` passed on 2026-05-21 after commit `d0fe1f6` deployed; hosted `workspace.js` returned `data-evidence-download="file"` on poll 3, fake `.test` text upload/download matched bytes, a >5MB fake `.test` resumable upload/download matched bytes, D1 upload/download audit rows were verified, denial guards passed, and dashboard output exposed only app-scoped download URLs.
- `last updated`: 2026-05-21 07:39 PT
...
- `artifact`: `docs/master-plan.md`, `docs/mvp-requirements-catalog.md`, `docs/production-surface-registry.md`, `docs/source-materials/production-content-crosswalk.md`, `scripts/check-production-surfaces.mjs`, `SC-007`, and active Figma file `z4t4tFPAKrMDh6pIYOeEw6` node `124:2` (`Prototype / 15 / Production app and public guide boundary`).
- `next action`: Broaden no-hidden-core-content coverage proof across every public guide route, then verify hosted post-deploy account-state/no-assignment markers and a live section-level permission-denied browser state. The non-Figma lane has now consumed Figma node `133:2` for local session-expired, disabled-account, reset-required, role-pending, permission-denied, and no-active-assignment workspace states; generated route inventory now classifies `workspace.html` as the canonical protected production app route; Figma node `124:2` records that route proof as consumed; and hosted Drive upload/download proof now passes.
- `acceptance check`: Public website mode toggle stays visible with source-aligned Student/Teacher content; normal public nav stays free of alpha/account/internal QA links; canonical `workspace.html` route requires auth and chooses screens from D1-backed role/scope, with credential-backed proof for local student login/session/evidence/upload/download/logout, local role routes, no-role role-pending UI, unassigned mentor no-assignment UI, source-executed disabled/reset/session/permission-denied UI, direct live signed-out/signed-in workspace proof, and live fake `.test` Drive upload/download proof.
- `evidence to close`: Route inventory now aligns with the registry for `workspace.html`: `docs/generated/production-route-inventory.md` classifies it as `senior-capstone-app` / `production` / `conditional`, and `tests/production-workflow-source.test.mjs` rejects the prior unknown row. Figma node `124:2` was updated on 2026-05-21 02:50 PT with shared plugin data key `senior_capstone/production_route_inventory_consumption_2026_05_21`; `use_figma` readback found 79 text nodes, zero collapsed/clipped text, zero direct-child overflow, and `get_screenshot` returned `783x1024` from original `1360x1779`. Commit `d0fe1f6` added storage-ID-redacted dashboard download URLs plus workspace evidence download/open-link actions; hosted `workspace.js` returned `data-evidence-download="file"` on poll 3; `npm run check:drive:live` passed fake `.test` text upload/download, >5MB resumable upload/download, D1 metadata/audit, denial, and leak checks. Remaining close evidence: passing `check:production-surfaces`, `check:generated-output-drift`, `check`, `tests/workspace-app.test.mjs`, credential-backed `tests/workspace-browser-smoke.test.mjs` against local Pages dev, in-app browser signed-in workspace/evidence/logout proof, in-app browser no-role role-pending proof, in-app browser unassigned mentor no-assignment proof, direct live signed-out/signed-in workspace proof, hosted post-deploy account-state marker proof, and live section-level permission-denied proof.
- `last updated`: 2026-05-21 07:39 PT
...
- `next action`: Remote migration `0007_archive_export_artifacts.sql` is applied/recorded and the general Drive live upload/download gate now passes. Capture hosted fake-account proof for the Drive-backed archive package upload/app-scoped download path; decide whether a separate signed Drive-link delivery is needed beyond scoped app downloads.
...
- `evidence to close`: Hosted fake-account proof for Drive-backed package upload/app-scoped download markers. First consumption slice landed previously: `/api/student/archive/readiness`, workspace Archive tab, explicit admin archive reason, truthful signed-download disabled state, `tests/archive-readiness.integration.test.mjs`, workspace tests/smoke, and in-app browser student Archive tab proof. The 2026-05-20 21:10 PT non-Figma slice added `export_artifacts`, scoped JSON archive manifest generation, authenticated manifest download, expired-package retry, workspace manifest link markers, and focused integration/source/workspace tests. The 2026-05-20 21:17 PT Figma slice added node `149:2` with provider-unavailable, signed-link, remote migration, and retention-policy route/data/permission states. The 2026-05-20 21:38 PT non-Figma slice partially consumed node `149:2` by provider-gating archive generation, auditing failed provider states, adding configurable retention metadata, and rendering workspace retention state. The 2026-05-20 22:09 PT non-Figma slice partially consumed node `151:2` by uploading redacted manifest JSON into Drive after provider probes, storing Drive file IDs only server-side, and exposing only Drive-package-ready markers/headers. The 2026-05-21 live Drive check proved general fake `.test` file-byte upload/download and >5MB resumable upload/download now pass against the Shared Drive root; archive-specific hosted proof still remains.
- `last updated`: 2026-05-21 07:39 PT
...
- `next action`: Remote migrations `0007` and `0008` are applied/recorded, and the general Drive live upload/download gate now passes. Capture hosted archive UI/API proof, and decide whether signed Drive links are required beyond the scoped app download endpoint.
...
- `next action`: Capture hosted fake-account proof for admin archive generation, Drive package upload markers, student archive download states, expired retry, and permission denial, then decide whether a separate signed Drive-link flow is needed.
...
- `evidence to close`: Hosted fake `.test` proof for admin queue, Drive package upload, student archive card, scoped download, expired retry, and permission denial using node `151:2` as the design contract. Repo-side partial consumption landed in commit `39c04b051f2d63eb81e513b679c656b4294d1586`: archive generation uploads a redacted package file to Drive after provider probes, stores the Drive ID only server-side, exposes safe package-ready markers, and keeps raw Drive IDs out of workspace/download responses. General live Drive proof now passes fake `.test` text upload/download and >5MB resumable upload/download after commit `d0fe1f6`; archive-specific hosted proof remains open.
- `last updated`: 2026-05-21 07:39 PT
...
- `next action`: Continue broadening route-level permission tests beyond the student dashboard, teacher review queue, submission, evidence-link/upload/download, and mentor-meeting proofs; decide whether email delivery or invitations are needed beyond one-time admin display credentials; and implement remaining first admin/progress workflow endpoints.
...
- `evidence to close`: Passing tests plus first vertical-slice endpoints that use the D1 schema and Google Drive evidence repository metadata without real student data. D1-backed alpha flow, state-machine tests, alpha framework/check/CI, deployment verification, first-admin bootstrap, fake role-account seeding, login proof, admin-only seed endpoint guard evidence, reset-required `/api/auth/complete-reset` credential rotation, admin reset initiation, active-user `/api/auth/change-password` credential rotation, admin `/api/admin/users/import` provisioning with generated reset-required temporary credentials, denied-role audit, reset-first import API proof, redacted audit tests, canonical workspace admin import UI marker proof, local Pages/D1 browser-smoke import/reset proof, setup-output memory-only UI proof, `/api/student/dashboard` unauthorized/denied/viewed audit coverage across student/mentor/teacher/admin/misc-admin scopes plus storage-ID-redacted download URLs, `/api/teacher/review-queue` unauthorized/denied/viewed audit coverage plus empty program-scope no-leak regression coverage, `/api/reviews/:submissionId/history` unauthorized/denied/viewed audit coverage, `/api/reviews/:submissionId/decision` unauthorized/denied reviewer audit coverage, `/api/submissions/:id/evidence` missing-session/denied/success audit coverage, `/api/submissions/:id/submit` unauthorized/denied/blocked/success audit coverage, `/api/evidence/:id/download` private streaming download coverage, hosted fake `.test` Drive text upload/download proof, hosted >5MB resumable upload/download proof, remote D1 upload/download audit verification, and `/api/mentor/meetings` read/mutation access audit coverage across student/mentor/teacher/admin/misc-admin scopes now exist; remaining endpoint permission tests, real-user credential delivery policy, hosted permission-state UI proof, upload progress/retry UX, Google Docs export cases, and archive-specific hosted proof remain before close.
- `last updated`: 2026-05-21 07:46 PT
