...
- `next action`: Rebuild should treat `/api/student/dashboard` route-level access auditing, `/api/teacher/review-queue` empty-scope/audit hardening, `/api/reviews/:submissionId/history` access auditing, `/api/reviews/:submissionId/decision` unauthorized/denied audit hardening, `/api/submissions/:id/evidence` evidence-link access auditing, `/api/submissions/:id/submit` submit access auditing, and `/api/submissions/:id` submission-detail access auditing as consumed, then use node `196:2` as the acceptance checklist for one remaining workflow endpoint family: evidence upload/download browser/live proof, archive/export, mentor meetings, presentation slots, dashboard aggregate readbacks, or hosted no-assignment plus section-level permission-denied states with fake `.test` accounts only.
...
- `next action`: Continue broadening route-level permission tests beyond the student dashboard and teacher review queue, resolve the Drive upload HTTP 403, decide whether email delivery or invitations are needed beyond one-time admin display credentials, and implement remaining first admin/progress workflow endpoints.
...
- `latest Figma handoff`: 2026-05-21 node `196:2` was added to consolidate the protected-record audit chain after node `190:2` marked `GET /api/submissions/:id` submission detail/readback access audit proof consumed. Next protected-record rebuild work should use node `196:2` as the acceptance checklist for evidence upload/download proof, mentor-meeting, presentation-slot, archive/export endpoints, dashboard aggregate readbacks, and hosted no-assignment plus section-level permission-denied UI proof. Node `151:2` remains the Drive archive delivery contract; it is partially consumed by the repo-side Drive package upload path, with hosted fake-account proof still blocked by Drive upload HTTP 403.
- `last updated`: 2026-05-21 06:16 PT
