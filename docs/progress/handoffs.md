...
- `next action`: Build the remaining export-delivery depth: real archive artifact generation, signed expiry or safe Drive link behavior, expired-download retry, provider-unavailable export generation states, retention handling, and hosted archive UI proof after deployment/Drive credentials.
...
- `evidence to close`: Follow-up rebuild commit for real export artifacts and signed/expired download behavior, plus focused tests and hosted proof. First consumption slice landed in this run: `/api/student/archive/readiness`, workspace Archive tab, explicit admin archive reason, truthful signed-download disabled state, `tests/archive-readiness.integration.test.mjs`, workspace tests/smoke, and in-app browser student Archive tab proof.
...
- `latest Figma handoff`: 2026-05-20 node `144:2` added celebration evidence, thank-you/mentor note, reflection/portfolio, archive request, expired download, provider-unavailable, and archive permission-denied states. It is now partially consumed by `/api/student/archive/readiness` and the workspace Archive tab; remaining implementation depth is real export artifact generation plus signed/expired download behavior. Node `139:2` was already consumed by the non-Figma workspace Presentation tab for persisted slot visibility and staff check-out/check-in controls.
- `last updated`: 2026-05-20 20:43 PT
