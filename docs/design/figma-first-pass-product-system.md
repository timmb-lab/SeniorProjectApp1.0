...
## Submission Detail Access Audit Handoff

The 2026-05-21 05:54 PT Figma pass added a submission-detail access audit handoff after rebuild proved `GET /api/submissions/:id` protected detail/readback audits.

- `Prototype / 29 / Submission detail access audit handoff`, node `190:2`, turns scoped submission and evidence-summary readback proof into route/data/permission implementation guidance.
- State annotations cover signed-out detail request, student-other detail denied, misc-admin broad access denied, student-own detail readback, mentor/teacher/admin scoped view, and storage identifier redaction.
- Route annotations name `GET /api/submissions/:id`, `/student/submissions/:id`, `/workspace`, `/api/submissions/:id/evidence`, `/api/submissions/:id/submit`, `/admin/audit`, and `/api/audit-events`.
- Record annotations name `User`, `UserRole`, `Session`, `Submission`, `EvidenceArtifact`, `RequirementProgress`, `Review`, `Comment`, `MentorAssignment`, `GroupMembership`, `StaffProgramAssignment`, and `AuditEvent`.
- Audit annotations name `submission_detail_unauthorized`, `submission_detail_denied`, and `submission_detail_viewed`; guardrails require redacted actor role scopes, `storageIdentifiersRedacted=true`, safe evidence summaries only, no raw Drive IDs, fake `.test` proof only, and production-safe 401/403 copy.
- Shared plugin data key `senior_capstone/submission_detail_access_audit_contract_2026_05_21` records consumed run `2026-05-21-0535-submission-detail-access-audit-mvp-012`, consumed implementation commit `48de622de03b1c1ea660ee8b613d6b23c3d139b4`, consumed closeout commit `427d9fd17d6493765fa853c9bb4289810c6c71fa`, 6 states, 7 routes, 12 records, 7 permission scopes, and 6 guardrails.
- Initial readback found fixed-width text nodes and horizontal rows collapsed to 1px height; targeted layout corrections expanded the frame to `1360x1381`. Final `use_figma` readback found 51 text nodes, zero collapsed text nodes, zero suspicious clipped text nodes, zero child overflow, 6 states, 7 routes, 12 records, 3 audit events, 7 permission scopes, and 6 guardrails. `get_design_context` and `get_screenshot` succeeded for node `190:2`; screenshot verification returned `1009x1024` from original `1360x1381`.

## Protected Record Audit Acceptance Board

The 2026-05-21 06:16 PT Figma pass added an acceptance-check board after no newer non-Figma proof existed beyond the submission-detail handoff.

- `Prototype / 30 / Protected record audit acceptance board`, node `196:2`, consolidates the protected-record audit chain from nodes `173:2`, `178:2`, `180:2`, `183:2`, `187:2`, and `190:2`.
- The board maps 7 consumed endpoint families: student dashboard, teacher review queue, review history, review decision, evidence-link metadata attach, submission submit, and submission detail/readback.
- The board names 6 remaining proof families for rebuild: evidence upload/download, mentor meetings, presentation slots, archive/export, dashboard aggregate readbacks, and hosted no-assignment plus section-level permission-denied UI proof.
- Shared plugin data key `senior_capstone/protected_record_acceptance_matrix_2026_05_21` records `ready_for_rebuild_followup`, 12 supporting MVP IDs, 6 consumed nodes, 7 role scopes, and 8 acceptance checks.
- Initial readback found 12 child-overflow containers from fixed-height rows; targeted row width, autosizing, and text-height corrections expanded the frame to `1360x2724`. Final `use_figma` readback found 122 text nodes, zero suspicious clipped text nodes, zero child overflow, consumed-node references present, and the remaining proof queue present. `get_design_context` and `get_screenshot` succeeded for node `196:2`; screenshot verification returned `512x1024` from original `1360x2724`.
...
- Full MVP alpha prototype page: node `98:2`, with review detail node `98:9` and student revision node `98:10` aligned to the implemented review-history endpoint, handoff frame `98:17` updated after primary alpha-console consumption, production boundary handoff node `124:2` distinguishing the public guide mode from the authenticated workspace route, workspace account edge-state handoff node `133:2`, presentation dashboard state handoff node `139:2`, celebration archive readiness handoff node `144:2`, archive provider/retention handoff node `149:2`, Drive archive delivery handoff node `151:2`, credential lifecycle handoff node `153:2`, admin import temporary credential handoff node `158:2`, admin import proof QA handoff node `163:2`, student dashboard access audit handoff node `173:2`, teacher review queue access audit handoff node `178:2`, review history/decision access audit handoff node `180:2`, evidence link access audit handoff node `183:2`, submission submit access audit handoff node `187:2`, submission detail access audit handoff node `190:2`, and protected-record audit acceptance board node `196:2`.

Exact next action:
- Rebuild should consume nodes `18:2`, `31:2`, `37:2`, `43:2`, `48:2`, `56:2`, `61:2`, `69:2`, `78:2`, `98:9`, `98:10`, `98:17`, `124:2`, `133:2`, `139:2`, `144:2`, `149:2`, `151:2`, `153:2`, `158:2`, `163:2`, `173:2`, `178:2`, `180:2`, `183:2`, `187:2`, `190:2`, and acceptance board `196:2` while scaffolding the accepted Cloudflare database/auth/progress/audit/export/meeting/presentation foundation. Review history, workspace account states, presentation dashboard states, initial archive manifests, admin import API behavior, student dashboard protected-access auditing, teacher review queue scope/audit behavior, review history/decision access auditing, evidence-link metadata access auditing, submission-submit access auditing, and submission-detail readback auditing are already partially consumed locally; next rebuild focus should use node `196:2` as the checklist for evidence upload/download proof, mentor-meeting, presentation-slot, archive/export, dashboard aggregate readbacks, and hosted permission UI proof. Node `151:2` remains the Drive-backed package or signed-link delivery handoff after the Drive upload HTTP 403 is fixed.
