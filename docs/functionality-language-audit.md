...
| 13 | Functionality | Site admin | Mentor Assignments coverage and active assignment rows | Mentor load and assignment rows now open the scoped `mentorUserId` active-assignment filter. | Preserve route-backed mentor load handoff. | Workload management. | `workspace.js`, `tests/workspace-app.test.mjs` | Manage/view only by role; no mutation change. | Site. | S | Complete | Filter state test. | Completed 2026-05-31. |
