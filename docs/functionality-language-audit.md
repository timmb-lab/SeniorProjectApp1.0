...
| `student` | Own dashboard, submissions/evidence, archive readiness, presentation scope, own record. | Student homepage now has a progress-first dashboard using real requirements/submissions/evidence/mentor data, teacher feedback from own submissions, submission version/status context, and phase-focused requirement filters. | No dedicated student feedback page or due-date timeline yet. |
...
- Site dashboard `Recent Activity` opens Audit only for Global Admin users with the backed audit surface; site-scoped users now keep the count as summary-only until a route-backed site audit destination exists.
...
| 13 | Functionality | Site admin | Mentor Assignments summary, coverage, and active assignment rows | Mentor load rows now open the scoped `mentorUserId` active-assignment filter, and the section summary tiles open active or missing-mentor filters. | Preserve route-backed mentor load handoff. | Workload management. | `workspace.js`, `tests/workspace-app.test.mjs`, dashboard/navigation verifiers | Manage/view only by role; no mutation change. | Site. | S | Complete | Filter state test. | Completed 2026-05-31. |
...
| 34 | Functionality | Site admin, global admin | Users & Access site assignment panel | The site-access API returned active assignment rows, but the workspace only showed save/remove forms. | Render current mentor, viewer, program teacher, administration, and site-admin assignments before the forms. | Admins can confirm current access before changing it. | `workspace.js`, `workspace.css`, `tests/workspace-app.test.mjs` | Uses only scoped `/api/site/access-assignments` response; no permission or mutation change. | Site. | S | Complete | Workspace render shows `data-site-access-assignment-summary="true"` before forms. | Completed 2026-05-31. |
| 34 | Functionality | Student | Phase detail absent | Progress details grouped, but students could not focus one phase at a time. | Add in-page phase-focused detail panels and filters. | Clear tasks. | student dashboard UI/API | Self only. | Program. | S | Complete | Workspace render proves phase focus and matching requirement open behavior. | Completed 2026-05-31 with phase focus controls and focused workspace coverage. |
...
| 34a | Functionality | Site admin, global admin | Users & Access action guidance | Assignment forms exposed Assign/Remove but used a generic save button and did not explain the removal boundary. | Clarify Assign vs Remove, note that removal should match a current row, and label the submit as an access change. | Admins can make safer assignment changes without thinking Remove deletes accounts or records. | `workspace.js`, `tests/workspace-app.test.mjs` | Copy only; no route, permission, mutation, tenant, site, program, or student visibility change. | Site. | XS | Complete | Workspace render shows `data-site-access-action-guidance` and `Save access change`. | Completed 2026-05-31. |
| 34b | Functionality | Site admin, global admin | Users & Access recent access history | Access changes were recorded in scoped audit rows, but Users & Access did not show the latest site access changes before admins used the forms. | Render a read-only recent access history panel from existing scoped site access audit rows, excluding admin-note text. | Admins can confirm the latest access changes without opening a broader audit view. | `functions/api/site/access-assignments.ts`, `workspace.js`, `tests/site-access-assignments.integration.test.mjs`, `tests/workspace-app.test.mjs` | Uses only existing `site_access_assignment` audit rows and the scoped Users & Access route; no permission or mutation change. | Site. | S | Complete | Route returns `history`, workspace shows `data-site-access-history="true"`, and admin notes stay hidden. | Completed 2026-05-31. |
...
| 2026-05-31 | Added route-backed filters to the authorized student-detail Timeline tab so staff and assigned mentors can narrow the existing scoped timeline to reviews, evidence, mentor meetings, presentation, archive, submissions, notes, or status changes without adding routes or broadening access. | `workspace.js`, `tests/workspace-app.test.mjs`, focused workspace/language/dashboard/navigation verifiers | Complete |
| 2026-05-31 | Added a scoped `studentId` filter to Operations readiness and student-detail Presentation/Archive actions that open Operations for the current student, so authorized staff and read-only viewers can inspect exact presentation/archive worklists without adding routes, fake controls, or access expansion. | `functions/_lib/site-operations-readiness.ts`, `workspace.js`, `tests/site-operations-readiness.integration.test.mjs`, `tests/workspace-app.test.mjs`, focused route/UI/dashboard/navigation verifiers | Complete |
| 2026-05-31 | Clarified exact-student Operations empty states so student-detail drill-downs with no matching presentation, archive, or readiness rows tell staff they are still filtered to the current student instead of sending them back to the broader directory. | `workspace.js`, `tests/workspace-app.test.mjs`, focused workspace/language/dashboard/navigation verifiers | Complete |
...
| 2026-05-31 | Added real `View student detail` actions to Program Teacher dashboard `Needs Review` rows, reusing the existing authorized student-detail drawer so teachers can move from a submitted/revision row into student context without a fake queue filter or new route. | `workspace.js`, `tests/workspace-app.test.mjs`, focused workspace/dashboard/navigation/language verifiers | Complete |
| 2026-05-31 | Added shareable selected-submission Review Queue state so authorized staff and read-only viewers can reload or share a scoped review queue URL that reopens the existing selected submission panel and protected history only when that submission is still present in the current scoped queue. | `workspace.js`, `tests/workspace-app.test.mjs`, `scripts/verify-review-queue-deeplinks.mjs`, focused workspace/review-queue/navigation verifiers | Complete |
| 2026-05-31 | Clarified stale Review Queue selected-submission links so shared URLs whose `submissionId` no longer appears in the current scoped queue show a clear "not visible" panel, clear the stale URL value, and avoid loading protected history. | `workspace.js`, `tests/workspace-app.test.mjs`, `scripts/verify-review-queue-deeplinks.mjs`, focused workspace/review-queue verifier | Complete |
| 2026-05-31 | Added mentor-load guidance to the site Mentor Assignments form, showing active assignment counts, load labels, and the lightest visible mentor before authorized staff save a one-student assignment. | `workspace.js`, `tests/workspace-app.test.mjs`, focused workspace/language/dashboard/navigation verifiers | Complete |
| 2026-05-31 | Added `View assignments` actions to Mentor Assignments mentor coverage rows, reusing the existing scoped `mentorUserId` and active-assignment filters so staff can jump from mentor load context to that mentor's current assignments without new routes or mutation controls. | `workspace.js`, `tests/workspace-app.test.mjs`, focused workspace/dashboard/navigation/language verifiers | Complete |
| 2026-05-31 | Added `View mentor load` actions to Mentor Assignments active assignment rows, reusing the existing scoped `mentorUserId` and active-assignment filters so staff can move from one student assignment back to that mentor's current load without new routes or mutation controls. | `workspace.js`, `tests/workspace-app.test.mjs`, focused workspace test/verifier | Complete |
| 2026-05-31 | Linked Mentor Assignments `Students With Mentors` and `Missing Mentors` summary tiles to the existing scoped active-assignment and unassigned-student filters, so staff and read-only viewers can jump from coverage counts to matching rows without new routes or mutation controls. | `workspace.js`, `tests/workspace-app.test.mjs`, `scripts/verify-dashboard-actions.mjs`, `scripts/verify-workspace-navigation-integrity.mjs` | Complete |
| 2026-05-31 | Added in-page Presentation schedule filters for all slots, ready-for-check-out, checked-out, checked-in, and outline follow-up using rows already returned by the scoped `/api/presentation-slots` route, so students, mentors, teachers, and admins can scan presentation-day work without new query params, routes, or permission changes. | `workspace.js`, `tests/workspace-app.test.mjs`, focused workspace test | Complete |
| 2026-05-31 | Added in-page Mentor Dashboard filters for all assigned students, revision follow-up, meeting attention, and presentation follow-up using rows already returned by the scoped `/api/mentor/dashboard` route, so mentors can focus their assigned-student list without new query params, routes, permissions, or data paths. | `workspace.js`, `tests/workspace-app.test.mjs`, focused workspace test | Complete |
| 2026-05-31 | Added a current-assignment summary to Users & Access from the existing scoped `/api/site/access-assignments` payload, so site admins can confirm active mentor, viewer, program teacher, administration, and site-admin access before using the save/remove forms. | `workspace.js`, `workspace.css`, `tests/workspace-app.test.mjs`, focused workspace test | Complete |
| 2026-05-31 | Clarified Users & Access assignment form guidance so site admins see Assign/Remove boundaries, current-row removal expectations, and a `Save access change` action before submitting existing scoped access changes. | `workspace.js`, `tests/workspace-app.test.mjs`, focused workspace/language/dashboard/navigation verifiers | Complete |
| 2026-05-31 | Added a read-only `Recent access changes` panel to Users & Access from existing scoped `site_access_assignment` audit rows, so site admins can review the latest access assignments/removals for this school without exposing admin-note text or opening a broader audit view. | `functions/api/site/access-assignments.ts`, `workspace.js`, `tests/site-access-assignments.integration.test.mjs`, `tests/workspace-app.test.mjs` | Complete |
| 2026-05-31 | Removed unsupported Global Admin-only `Audit` and `Archive / Exports` navigation from site-admin workspaces, and made Site Dashboard `Recent Activity` summary-only when there is no backed audit surface, so site-scoped users no longer land on unavailable sections. | `workspace.js`, `tests/workspace-app.test.mjs` | Complete |
| 2026-05-31 | Added real student-home next-step actions from the existing scoped `nextSteps`, requirement, and submission data, so students can open the matching requirement and reuse current add-evidence/send-for-review actions without new routes or access expansion. | `functions/api/student/dashboard.ts`, `workspace.js`, `tests/student-dashboard-access.integration.test.mjs`, `tests/workspace-app.test.mjs` | Complete |
| 2026-05-31 | Added real `View timeline` actions to student Submitted Work rows, reusing the existing student-safe `/api/reviews/:submissionId/history` route so students can open version/status/feedback history from the submission list without new routes or access expansion. | `workspace.js`, `tests/workspace-app.test.mjs`, `docs/student-progress-dashboard.md` | Complete |
