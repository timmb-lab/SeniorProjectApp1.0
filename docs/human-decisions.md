...
- `current recommendation`: For the local-account-only 1.0 setup, authorized admins may use the admin workspace one-time display UI to create local pending-reset accounts, then distribute setup credentials through Bryan's chosen out-of-band procedure. Before a district SSO or live pilot rollout, Bryan still needs to choose one of: keep manual one-time credential distribution, implement an invitation/email delivery path, or switch to pre-provisioned SSO. Do not put temporary credential values in docs, Figma, screenshots, logs, audit metadata, commits, or chat.
...
- `2026-05-21 account reset update`: Bryan approved a destructive account/data reset path that clears old users, sessions, password credentials, auth identity links, tenant-user memberships, OAuth states, role assignments, and user-linked workflow/test rows, then recreates only two local-auth global admins: `bryan@learntechonline.com` and `bryan.timm89@gmail.com`. Both are Bryan Timm, global `admin`, local username/password, and reset-required on first use. One-time setup credentials and reset backups must live only in ignored `.secrets/` files. Old `bryan@thecapstoneapp.com` and fake `.test` accounts are cleared and not recreated by the reset path. Google Workspace SSO remains separate and not required for these local admins.
- `2026-06-07 local-only 1.0 update`: Bryan requested SSO disabled for now and local accounts enabled so he can build test accounts. Current code now treats `AUTH_MODE=hardened_username_password` with local login enabled as the approved account creation mode: real local accounts can be imported by authorized admins, setup passwords are returned once in the no-store admin response, and Google SSO account creation is rejected while SSO is disabled.
