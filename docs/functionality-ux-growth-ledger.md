...
## Run 2026-05-31 06:34 PT

- Starting SHA: `d05cbaefcbb8a5aa06e0112ca508603c47d25956`
- Ending SHA: pending closeout commit; final hash is in the completion report
- Branch: `main`
- Branch policy: work stayed on clean local `main`; local `main` was seven commits ahead of `origin/main`, `origin/main` was not ahead, and no push was run
- Ladder level targeted: `LEVEL_5_REVIEW_AND_INTERVENTION_QUEUES` with `LEVEL_1_NAVIGABLE_DASHBOARDS` URL-state support
- Backlog item: `review-queue-selected-submission-deeplink-feasibility`; supports `MVP-015` through `MVP-018`, `MVP-032`, and `MVP-033`
- Work order selected: Add shareable selected-submission state to the Review Queue.
- Selection reason: Current source already had a real Review Queue selected-submission panel and protected `/api/reviews/:submissionId/history` route, but shareable URL state restored only filters. This slice improves real queue continuity without backend query semantics, new routes, mutations, fake links, or access expansion.
- Candidate scoring summary:

| Candidate | Ladder Level | Roles | Impact | Safety | Testability | Size | Score | Decision |
|---|---|---|---:|---:|---:|---|---:|---|
| Review Queue selected-submission URL state | `LEVEL_5_REVIEW_AND_INTERVENTION_QUEUES` | program_teacher, site staff, viewer | 4 | 5 | 5 | S | 56 | selected |
...
| Public route browser/mobile visual QA | `LEVEL_9_AUTONOMOUS_QUALITY_IMPROVEMENT` | public students, teachers | 3 | 4 | 3 | M | 40 | not selected: lower protected-app value |
| Student archive hosted proof | `LEVEL_8_REPORTING_AND_OPERATIONAL_READINESS` | student | 3 | 3 | 3 | M | 36 | deferred: hosted/live-data boundary |
| Program Teacher Needs Review row detail action | `LEVEL_2_STUDENT_DETAIL_DEPTH` | program_teacher | 3 | 5 | 5 | XS | 36 | rejected: completed previous run |
| Review Queue filter empty-state copy | `LEVEL_5_REVIEW_AND_INTERVENTION_QUEUES` | program_teacher, site staff | 2 | 5 | 4 | XS | 40 | rejected: less functional than URL restoration |

- User-facing improvement: Authorized Review Queue users can reload or share a filtered Review Queue URL that reopens the selected submission panel and protected history when the requested submission remains visible in the scoped queue.
- Roles affected: `platform_admin`, `admin`, `org_admin`, `site_admin`, `viewer`, and `program_teacher`; no student, mentor, misc_admin, tenant, site, program, or record visibility was broadened.
- Files changed: `workspace.js`, `tests/workspace-app.test.mjs`, `scripts/verify-review-queue-deeplinks.mjs`, `docs/functionality-language-audit.md`, `docs/mvp-requirements-catalog.md`, `docs/progress/run-log.md`, `docs/progress/runs/2026-05-31-0634-review-queue-selected-submission-url.json`, `docs/functionality-ux-growth-ledger.md`, `automation/state/functionality-ux-growth-state.json`
- Tests/verifiers added or updated: workspace coverage proves `submissionId` URL restoration loads `/api/reviews/:submissionId/history` only after scoped queue rows load, syncs selected state into URLs, restores on popstate, and clears the selected submission when filters are reset; the Review Queue deeplink verifier now guards selected-submission URL support.
- Validation commands:
  - Focused passed before docs/state closeout: `node --test tests/workspace-app.test.mjs`; `npm run verify:review-queue-deeplinks`; `npm run verify:dashboard-actions`; `npm run verify:workspace-navigation`; `npm run verify:functionality-language`
  - Final passed: JSON parse for state/run manifest, `node --test tests/functionality-language-audit.test.mjs`, `npm run verify:functionality-ux-automation`, `git diff --check`, `npm run check:production-surfaces`, `npm run check:route-inventory`, `npm run test`, `npm run typecheck`, `npm run check`
- Validation result: passed; `git diff --check` reported only CRLF normalization warnings, with no whitespace errors.
- Commit: pending closeout commit
- Push status: not pushed
- Deferred items: Review Queue missing-submission semantics, hosted section-level permission proof, Student Directory missing-evidence filter, archive retry/regenerate controls, mentor workload threshold guidance, org rollups, student archive hosted proof, and public-route browser/mobile QA remain deferred.
- New backlog items: none
- Next recommended work order: inspect whether Review Queue selected-submission state needs a small "selected row no longer visible" empty-panel message after more hosted/browser proof; otherwise wait on missing-submission route semantics before adding new queue filters.
- Do-not-repeat notes: do not re-add Review Queue selected-submission URL restoration unless regression removes `reviewQueueSelectionFromSearchParams()`, `submissionId` URL sync, or the focused popstate/history coverage.
- Ladder Handoff:
  - Targeted Level: `LEVEL_5_REVIEW_AND_INTERVENTION_QUEUES`
  - Advanced: yes
  - Evidence: Review Queue URLs can now carry `submissionId`; the workspace restores the selected panel only when the scoped queue contains that submission and then loads the existing protected review-history route.
  - Unlocks: Staff can share or reload exact review context without inventing missing-submission queue rows or exposing unscoped submission history.
  - Next: selected-row no-longer-visible guidance only if evidence shows confusion; otherwise defer Review Queue missing-submission work until backend queue semantics exist.
...
## Run 2026-05-31 09:04 PT

- Starting SHA: `b416da38c4a7a22f3bc249af34783bce2e433a8d`
- Ending SHA: pending closeout commit; final hash is in the completion report
- Branch: `main`
- Branch policy: work stayed on clean local `main`; local `main` was ten commits ahead of `origin/main`, `origin/main` was not ahead, and no push was run
- Ladder level targeted: `LEVEL_3_MENTOR_ASSIGNMENT_WORKFLOW` with `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` site-operations support
- Backlog item: `mentor-coverage-row-assignment-filter`; supports `MVP-017`, `MVP-032`, and `MVP-033`
- Work order selected: Add route-backed mentor coverage row filters inside Mentor Assignments.
- Selection reason: The previous run added mentor-load guidance before assignment, but current Mentor Assignments coverage rows still made staff manually reselect a mentor to inspect current assignments. The existing route, URL state, and role gating already supported a safe `mentorUserId` drill-down.
- Candidate scoring summary:

| Candidate | Ladder Level | Roles | Impact | Safety | Testability | Size | Score | Decision |
|---|---|---|---:|---:|---:|---|---:|---|
| Mentor coverage row assignment filter | `LEVEL_3_MENTOR_ASSIGNMENT_WORKFLOW` | site_admin, org_admin, viewer, program_teacher | 4 | 5 | 5 | XS | 55 | selected |
...
| Review Queue stale selected-submission guidance | `LEVEL_5_REVIEW_AND_INTERVENTION_QUEUES` | program_teacher, site staff, viewer | 3 | 5 | 5 | XS | 36 | rejected: completed previous run |

- User-facing improvement: Mentor Assignments users can click `View assignments` on a mentor coverage row to open the same scoped worklist filtered to that mentor's active assignments, instead of manually selecting that mentor in the filter bar.
...
- Files changed: `workspace.js`, `tests/workspace-app.test.mjs`, `docs/functionality-language-audit.md`, `docs/progress/run-log.md`, `docs/progress/runs/2026-05-31-0904-mentor-coverage-row-filter.json`, `docs/functionality-ux-growth-ledger.md`, `automation/state/functionality-ux-growth-state.json`
- Tests/verifiers added or updated: workspace coverage proves the mentor coverage row action renders, reloads `/api/site/mentor-assignments` with `mentorUserId` and `status=active`, and syncs shareable URL state.
- Validation commands:
  - Focused passed before docs/state closeout: `node --test tests/workspace-app.test.mjs`; `npm run verify:functionality-language`; `npm run verify:dashboard-actions`; `npm run verify:workspace-navigation`
  - Final passed: JSON parse for state/run manifest, `node --test tests/functionality-language-audit.test.mjs`, `npm run verify:functionality-ux-automation`, `git diff --check`, `npm run check:production-surfaces`, `npm run check:route-inventory`, `npm run test`, `npm run typecheck`, `npm run check`
- Validation result: passed; `git diff --check` reported only CRLF normalization warnings, with no whitespace errors.
...
- Deferred items: Review Queue missing-submission semantics, hosted section-level permission proof, Student Directory missing-evidence filter, archive retry/regenerate controls, mentor workload threshold policy, org rollups, student archive hosted proof, and public-route browser/mobile QA remain deferred.
- New backlog items: none
- Next recommended work order: inspect whether active Mentor Assignments rows should expose a safe `View mentor load` return action only if it reuses the current scoped `mentorUserId` filter without adding mutation behavior; otherwise keep missing-submission Review Queue work deferred until backend rows exist.
- Do-not-repeat notes: do not re-add Mentor Assignments mentor coverage row filtering unless regression removes `data-mentor-assignment-action="filter-mentor"` or the focused workspace coverage.
- Ladder Handoff:
  - Targeted Level: `LEVEL_3_MENTOR_ASSIGNMENT_WORKFLOW`
  - Advanced: yes
  - Evidence: Mentor Assignments coverage rows now filter the existing scoped route to one mentor's active assignments with shareable URL state.
  - Unlocks: Staff can move from load context to exact current assignments before making coverage decisions.
  - Next: only add return/load-context affordances if they reuse existing filters; defer threshold-driven workload controls until policy is explicit.
...
## Run 2026-05-31 14:05 PT

- Starting SHA: `8291b758c5fed956b450be542b06fcdb36761612`
- Ending SHA: pending closeout commit; final hash is in the completion report
- Branch: `main`
- Branch policy: work stayed on clean local `main`; local `main` was one commit ahead of `origin/main`, `origin/main` was not ahead, and no push was run
- Ladder level targeted: `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` with `LEVEL_7_AUDITABILITY_AND_TRUST` access-management support
- Backlog item: `site-access-current-assignment-summary`; supports `MVP-004`, `MVP-006`, `MVP-032`, and `MVP-033`
- Work order selected: Render current site access assignments before Users & Access management forms.
- Selection reason: A recent users/access V5 change added a scoped `/api/site/access-assignments` payload with active assignment rows, but the workspace only showed save/remove forms. Showing current scoped access before forms improves real admin workflow and trust without new routes, mutations, role grants, or data exposure.
- Candidate scoring summary:

| Candidate | Ladder Level | Roles | Impact | Safety | Testability | Size | Score | Decision |
|---|---|---|---:|---:|---:|---|---:|---|
| Site access current assignment summary | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | site_admin, global_admin | 4 | 5 | 5 | S | 56 | selected |
| Hosted section-level permission proof | `LEVEL_9_AUTONOMOUS_QUALITY_IMPROVEMENT` | all protected roles | 4 | 3 | 3 | M | 39 | deferred: hosted fake-account/audit writes not allowed in this lane run |
| Review Queue missing-submission semantics | `LEVEL_5_REVIEW_AND_INTERVENTION_QUEUES` | program_teacher, site staff | 5 | 2 | 3 | M | 34 | deferred: backend queue rows absent |
...
| Site access assignment history timeline | `LEVEL_7_AUDITABILITY_AND_TRUST` | site_admin, global_admin | 4 | 3 | 3 | M | 38 | deferred: audit-query UI shape needs review |
| Site admin assignment removal guidance | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | site_admin | 3 | 4 | 4 | S | 43 | not selected: summary is prerequisite and lower risk |
| Admin recent audit deep link | `LEVEL_7_AUDITABILITY_AND_TRUST` | admin, site_admin | 4 | 3 | 3 | M | 38 | deferred: audit destination shape needs review |
| Archive retry/regenerate controls | `LEVEL_8_REPORTING_AND_OPERATIONAL_READINESS` | site_admin, admin | 5 | 2 | 3 | M | 34 | rejected: mutation policy not approved |
| Mentor workload threshold filter | `LEVEL_3_MENTOR_ASSIGNMENT_WORKFLOW` | site staff, viewer | 4 | 3 | 3 | M | 39 | deferred: threshold policy unclear |
| Org admin all-sites rollup | `LEVEL_8_REPORTING_AND_OPERATIONAL_READINESS` | org_admin | 5 | 2 | 2 | L | 29 | rejected: aggregate backend design absent |
...
- User-facing improvement: Site admins now see active mentor, viewer, program teacher, administration, and site-admin access before the save/remove forms in Users & Access.
...
- Files changed: `workspace.js`, `workspace.css`, `tests/workspace-app.test.mjs`, `docs/functionality-language-audit.md`, `docs/progress/run-log.md`, `docs/progress/runs/2026-05-31-1405-site-access-assignment-summary.json`, `docs/functionality-ux-growth-ledger.md`, `automation/state/functionality-ux-growth-state.json`
- Tests/verifiers added or updated: workspace coverage proves `data-site-access-assignment-summary="true"` renders before assignment forms, resolves user/program/site labels from the scoped payload, shows role-safe current-access notes, and does not expose secret/storage fields.
- Validation commands:
  - Focused passed before docs/state closeout: `node --test tests/workspace-app.test.mjs`
  - Final passed: JSON parse for state/run manifest, `node --test tests/functionality-language-audit.test.mjs`, `npm run verify:functionality-language`, `npm run verify:dashboard-actions`, `npm run verify:workspace-navigation`, `npm run verify:functionality-ux-automation`, `git diff --check`, `npm run check:production-surfaces`, `npm run check:route-inventory`, `npm run test`, `npm run typecheck`, `npm run check`
- Validation result: passed; `git diff --check` reported only CRLF normalization warnings, with no whitespace errors.
- Commit: pending closeout commit
- Push status: not pushed
- Deferred items: hosted section-level permission proof, Review Queue missing-submission semantics, Presentation schedule URL-state filters, Mentor Dashboard URL-state filters, archive retry/regenerate controls, mentor workload threshold policy, org rollups, student guided requirement write path, and public-route browser/mobile QA remain deferred.
- New backlog items: consider a read-only site access assignment history timeline only after audit destination shape and role visibility are explicitly reviewed.
- Next recommended work order: run hosted section-level permission proof when hosted fake-account proof and audit-event writes are allowed; otherwise inspect whether access-assignment history can be safely exposed from existing audit rows without broadening visibility.
- Do-not-repeat notes: do not re-add the Users & Access current assignment summary unless `data-site-access-assignment-summary="true"` or the focused workspace coverage regresses.
- Ladder Handoff:
  - Targeted Level: `LEVEL_4_ROLE_SPECIFIC_WORKSPACES`
  - Advanced: yes
  - Evidence: Users & Access now renders current scoped assignment rows before save/remove forms from the existing `/api/site/access-assignments` response.
  - Unlocks: Site admins can verify current access before changing it; a future access-history view can build on this read-only summary if audit visibility is approved.
  - Next: hosted section-level permission proof when allowed; otherwise review access-assignment audit history shape before exposing any timeline.
  ...
  - Do not repeat: current assignment summary is complete unless marker/test coverage regresses.
...
## Run 2026-05-31 14:33 PT

- Starting SHA: `dad5380754ac523564fb595bffe21c83eaca5d54`
- Ending SHA: pending closeout commit; final hash is in the completion report
- Branch: `main`
- Branch policy: work stayed on clean local `main`; local `main` was two commits ahead of `origin/main`, `origin/main` was not ahead, and no push was run
- Ladder level targeted: `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` with `LEVEL_7_AUDITABILITY_AND_TRUST` access-management support
- Backlog item: `site-access-action-guidance`; supports `MVP-004`, `MVP-006`, `MVP-007`, `MVP-032`, and `MVP-033`
- Work order selected: Clarify Users & Access Assign/Remove guidance and submit copy.
- Selection reason: The previous run added a current-access summary before the Users & Access forms. Current source still showed a generic `Save assignment` action while each form exposed both `Assign` and `Remove`, and it did not tell site admins that removal should match an active row above and does not delete accounts, records, or student work. This was the safest continuation because it improves a real admin workflow without route, permission, mutation, tenant, site, program, or student visibility changes.
- Candidate scoring summary:

| Candidate | Ladder Level | Roles | Impact | Safety | Testability | Size | Score | Decision |
|---|---|---|---:|---:|---:|---|---:|---|
| Site access Assign/Remove guidance | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | site_admin, global_admin | 4 | 5 | 5 | XS | 55 | selected |
| Presentation schedule URL state | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | student, mentor, program_teacher, admin | 4 | 4 | 4 | M | 44 | not selected: shareable need still unproven |
...
- User-facing improvement: Site admins now see action-specific Assign/Remove guidance in Users & Access, plus a safer `Save access change` button, before submitting existing scoped access assignment changes.
- Roles affected: `site_admin`, `global_admin`, `admin`, and `platform_admin` where Users & Access is already available; no student, mentor, viewer, program teacher, tenant, site, program, or student visibility was broadened.
- Files changed: `workspace.js`, `tests/workspace-app.test.mjs`, `docs/functionality-language-audit.md`, `docs/progress/run-log.md`, `docs/progress/runs/2026-05-31-1433-site-access-action-guidance.json`, `docs/functionality-ux-growth-ledger.md`, `automation/state/functionality-ux-growth-state.json`
- Tests/verifiers added or updated: workspace coverage proves `data-site-access-action-guidance` renders for mentor-student and program-teacher forms, removal copy is clear, and the submit label is `Save access change`.
- Validation commands:
  - Focused passed before docs/state closeout: `node --test tests/workspace-app.test.mjs`; `npm run verify:functionality-language`; `npm run verify:dashboard-actions`; `npm run verify:workspace-navigation`
  - Final passed: JSON parse for state/run manifest, `node --test tests/functionality-language-audit.test.mjs`, `npm run verify:functionality-ux-automation`, `git diff --check`, `npm run check:production-surfaces`, `npm run check:route-inventory`, `npm run test`, `npm run typecheck`, `npm run check`
- Validation result: passed; `git diff --check` reported only CRLF normalization warnings, with no whitespace errors.
- Commit: pending closeout commit
- Push status: not pushed
- Deferred items: hosted section-level permission proof, Review Queue missing-submission semantics, Presentation schedule URL-state filters, Mentor Dashboard URL-state filters, site access assignment history timeline, archive retry/regenerate controls, mentor workload threshold policy, org rollups, and student guided requirement write path remain deferred.
- New backlog items: none
- Next recommended work order: run hosted section-level permission proof when hosted fake-account proof and audit-event writes are allowed; otherwise review access-assignment audit history shape before exposing any timeline.
- Do-not-repeat notes: do not re-add Users & Access Assign/Remove guidance unless `data-site-access-action-guidance` or `Save access change` regresses.
- Ladder Handoff:
  - Targeted Level: `LEVEL_4_ROLE_SPECIFIC_WORKSPACES`
  - Advanced: yes
  - Evidence: Users & Access assignment forms now explain Assign/Remove boundaries and use `Save access change` while preserving the existing scoped `/api/site/access-assignments` mutation path.
  - Unlocks: Site admins can make safer access changes from the visible current-access summary; future access-history work can stay focused on read-only history shape.
  - Next: hosted section-level permission proof when allowed; otherwise review access-assignment audit history visibility before exposing a timeline.
  - Blockers: hosted permission proof needs safe hosted fake-account/audit runtime; access-history UI needs audit visibility review; missing-submission queue rows do not exist; archive retry/regenerate needs mutation policy.
...
## Run 2026-05-31 17:37 PT

- Starting SHA: `68a1fa9c0dd15c3dda83c879e76398bf29da96b8`
- Ending SHA: pending closeout commit; final hash is in the completion report
- Branch: `main`
- Branch policy: work stayed on clean local `main`; local `main` remained ahead of `origin/main`, `origin/main` was not ahead, and no sync or push was run
- Ladder level targeted: `LEVEL_6_STUDENT_PROGRESS_DRILL_DOWN`
- Backlog item: `student-upcoming-deadlines-panel`; supports the student planning path in `docs/student-progress-dashboard.md`
- Work order selected: Add a due-date-first `Upcoming deadlines` panel to the existing student dashboard from the current requirement payload.
- Selection reason: The previous handoff explicitly pointed at a due-date-first student view when hosted proof stays blocked. Current `workspace.js` already loaded due labels, due dates, status, next actions, requirement ids, submission ids, and evidence counts through `/api/student/dashboard`, but students still had to scan multiple phase groups to find what was due next. This was the safest next slice because it reused existing student-only data and the current in-page requirement actions without adding routes, filters, or access expansion.
- Candidate scoring summary:

| Candidate | Ladder Level | Roles | Impact | Safety | Testability | Size | Score | Decision |
|---|---|---|---:|---:|---:|---|---:|---|
| Student upcoming deadlines panel | `LEVEL_6_STUDENT_PROGRESS_DRILL_DOWN` | student | 5 | 5 | 5 | S | 58 | selected |
| Student feedback compare or filter controls | `LEVEL_6_STUDENT_PROGRESS_DRILL_DOWN` | student | 4 | 4 | 4 | M | 44 | not selected: due-date-first planning was the earlier gap |
| Viewer vs manager Operations copy branch | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | viewer, site staff | 3 | 5 | 5 | XS | 43 | not selected: lower product depth than the student planning gap |
| Presentation schedule URL state | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | student, mentor, teacher, admin | 3 | 4 | 4 | S | 41 | deferred: add only if shareable filter links are clearly needed |
| Mentor Dashboard URL state | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | mentor | 3 | 4 | 4 | S | 40 | deferred: add only if mentors need shareable focus filters |
| Hosted section-level permission proof | `LEVEL_9_AUTONOMOUS_QUALITY_IMPROVEMENT` | all protected roles | 4 | 3 | 3 | M | 39 | deferred: hosted fake-account proof and audit writes are not allowed in this run |
| Site-scoped audit destination | `LEVEL_7_AUDITABILITY_AND_TRUST` | site_admin, org_admin | 4 | 3 | 4 | M | 38 | deferred: route shape and visibility review still needed |
...
- User-facing improvement: Students now get a due-date-first planning panel that surfaces the nearest incomplete deadline before the longer phase checklist, while reusing the same safe `Open requirement`, `Add evidence`, and submit-for-review actions they already had elsewhere in the student workspace.
- Roles affected: `student`
- Files changed: `workspace.js`, `tests/workspace-app.test.mjs`, `docs/student-progress-dashboard.md`, `docs/functionality-language-audit.md`, `docs/progress/run-log.md`, `docs/progress/runs/2026-05-31-1737-student-upcoming-deadlines.json`, `docs/functionality-ux-growth-ledger.md`, `automation/state/functionality-ux-growth-state.json`
- Tests/verifiers added or updated: `tests/workspace-app.test.mjs` now proves the new panel renders, orders incomplete deadline rows safely, excludes completed requirements, and preserves the current requirement actions.
- Validation commands:
  - Focused passed before docs/state closeout: `node --test tests/workspace-app.test.mjs`; `npm run verify:functionality-language`
  - Final passed: `node --test tests/functionality-language-audit.test.mjs`, JSON parse for `automation/state/functionality-ux-growth-state.json` and `docs/progress/runs/2026-05-31-1737-student-upcoming-deadlines.json`, `git diff --check`, `npm run verify:dashboard-actions`, `npm run verify:workspace-navigation`, `npm run verify:functionality-ux-automation`, `npm run check:production-surfaces`, `npm run check:route-inventory`, `npm run test`, `npm run typecheck`, `npm run check`
- Validation result: passed; `git diff --check` reported only CRLF normalization warnings, with no whitespace errors.
- Commit: pending closeout commit
- Push status: not pushed
- Deferred items: hosted section-level permission proof, site-scoped audit destination, Review Queue missing-submission semantics, Presentation schedule URL-state filters, Mentor Dashboard URL-state filters, student mentor support workflow shape, and site-admin assignment removal workflow remain deferred.
- New backlog items: none
- Next recommended work order: run hosted section-level permission proof when hosted fake-account proof and audit-event writes are allowed; otherwise inspect compare or filter affordances for the existing student-safe feedback timeline before adding a new student page.
- Do-not-repeat notes: do not re-add the student upcoming-deadlines panel unless regression removes `data-student-deadlines-panel="true"` or the due-date-first rows stop reusing the current requirement actions.
- Ladder Handoff:
  - Targeted Level: `LEVEL_6_STUDENT_PROGRESS_DRILL_DOWN`
  - Advanced: yes
  - Evidence: the student workspace now renders `data-student-deadlines-panel="true"` with due-date-first rows built from the current dashboard payload, and focused coverage proves the rows keep the existing requirement actions.
  - Unlocks: the student dashboard now has both priority-first and due-date-first entry points without creating a new route or calendar page.
  - Next: hosted permission proof when allowed; otherwise inspect compare or filter affordances for the existing student-safe feedback timeline.
...
## Run 2026-06-03 00:07 PT

- Starting SHA: `83b532a3bc23e042cd5d37772c6c94bdbdee82a0`
- Ending SHA: pending closeout commit; final hash is reported after commit
- Branch: `main`
- Branch policy: started from clean local `main` ahead of `origin/main` by seven earlier in-lane commits; this run stayed local-only and did not push
- Ladder level targeted: `LEVEL_4_ROLE_SPECIFIC_WORKSPACES`
- Backlog item: remaining Global Admin route-refresh handoff inside `docs/functionality-language-audit.md` and `docs/product/demo-role-readiness.md`
- Work order selected: Add route-backed site, program, and cohort labels to Global Admin recent role assignments.
- Selection reason: current repo evidence showed a real protected-workspace gap, not speculative polish. `workspace.js` already surfaced recent role assignments inside `Users & Access`, but `adminRoleAssignmentScopeText()` still guessed labels from the currently selected school's access payload. That made cross-school or off-site program grants fall back to raw ids or current-site-only context even though `/api/admin/role-assignments` was already the protected source of truth and could safely provide readable scope names.
- Candidate scoring summary:

| Candidate | Ladder Level | Roles | Impact | Safety | Testability | Size | Score | Decision |
|---|---|---|---:|---:|---:|---|---:|---|
| Role-assignment scope-name enrichment | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | `global_admin` | 5 | 5 | 5 | XS | 60 | selected |
| Broader Global Admin route refresh across Recent Audit, Students, Presentation, Programs, and Users & Access | `LEVEL_9_AUTONOMOUS_QUALITY_IMPROVEMENT` | `global_admin` | 4 | 4 | 3 | M | 45 | deferred: broader than one safe hourly slice |
| Filtered Audit cross-school proof refresh | `LEVEL_7_AUDITABILITY_AND_TRUST` | `global_admin` | 4 | 3 | 3 | M | 37 | deferred: needs a tighter locally provable scenario first |
| Admin mentor-meeting exact destination | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | `global_admin` | 4 | 2 | 2 | M | 30 | deferred: current workspace still lacks a precise meeting-follow-up destination |
| Program Teacher mentor-meeting follow-up destination | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | `program_teacher` | 4 | 3 | 3 | M | 40 | deferred: no precise scoped destination is proven |
| Hosted section-level permission proof | `LEVEL_9_AUTONOMOUS_QUALITY_IMPROVEMENT` | all protected roles | 4 | 3 | 2 | M | 35 | deferred: hosted runtime is outside this local batch |
| Review Queue missing-submission semantics | `LEVEL_5_REVIEW_AND_INTERVENTION_QUEUES` | `program_teacher`, `site_admin` | 5 | 2 | 3 | M | 34 | deferred: backend queue rows are still absent |
...
- User-facing improvement: Global Admin recent role assignments now show route-backed school, program, and cohort names even when no current school is selected, instead of falling back to raw scope ids or current-site-only guesses.
- Roles affected: `global_admin`
- Files changed: `functions/api/admin/role-assignments.ts`, `workspace.js`, `tests/admin-role-assignments.integration.test.mjs`, `tests/workspace-app.test.mjs`, `docs/functionality-language-audit.md`, `docs/product/demo-role-readiness.md`, `docs/progress/run-log.md`, `docs/progress/runs/2026-06-03-0007-role-assignment-scope-names.json`, `docs/functionality-ux-growth-ledger.md`, `automation/state/functionality-ux-growth-state.json`
- Tests/verifiers added or updated: `tests/admin-role-assignments.integration.test.mjs` now proves `/api/admin/role-assignments` returns readable site, program, and cohort names; `tests/workspace-app.test.mjs` now proves the Global Admin disclosure keeps those names readable even when `Users & Access` still requires picking a school for site-scoped access management.
- Validation commands:
  - `node --check workspace.js`
  ...
  - `npm run test`
  - `npm run check`
  - `node -e "JSON.parse(require('fs').readFileSync('automation/state/functionality-ux-growth-state.json','utf8')); JSON.parse(require('fs').readFileSync('docs/progress/runs/2026-06-03-0007-role-assignment-scope-names.json','utf8')); console.log('json ok')"`
  - `git diff --check`
- Validation result: passed; focused route and workspace coverage passed before docs/state closeout, and the full validation ladder passed after the closeout files were updated.
- Commit: pending closeout commit
- Push status: not pushed
...
- New backlog items: none
- Next recommended work order: Continue the bounded Global Admin route refresh only where the current workspace can prove an exact destination; preserve route-backed role-assignment labels and keep mentor-meeting follow-up summary-only until a precise backed path exists.
- Do-not-repeat notes: do not fall back to current-site-only scope guesses in recent role assignments when the protected route already provides readable scope names; do not surface raw site, program, or cohort ids in the Global Admin role-assignment panel when a route-backed scope label is available; and do not broaden Site Admin or other site-scoped workspaces with this Global Admin-only disclosure.
- Ladder Handoff:
  - Targeted Level: `LEVEL_4_ROLE_SPECIFIC_WORKSPACES`
  - Advanced: yes
  - Evidence: `/api/admin/role-assignments` now returns scope names for site, program, and cohort grants, and the `Users & Access` disclosure prefers those protected labels before any current-school fallback.
  - Unlocks: the next Global Admin pass can evaluate the remaining route-refresh and boundary gaps without rediscovering raw scope ids inside the role-assignment panel.
  - Next: inspect `workspace.js` around `renderAdminRoleAssignmentsBody()` and the remaining exact Global Admin handoffs only if a current destination is proven.
  - Blockers: mentor-meeting follow-up still lacks a precise destination; hosted permission proof still needs allowed runtime; Review Queue missing-submission semantics still need backend evidence.
...
## Run 2026-06-03 01:11 PT

- Starting SHA: `fda106a5b9d09c98b6ed298dee2fa8cd28ff1628`
- Ending SHA: pending closeout commit; final hash is reported after commit
- Branch: `main`
- Branch policy: started from clean local `main` ahead of `origin/main` by nine earlier in-lane commits; this run stayed local-only and did not push
- Ladder level targeted: `LEVEL_4_ROLE_SPECIFIC_WORKSPACES`
- Backlog item: remaining Global Admin role-assignment handoff depth inside `docs/functionality-language-audit.md` and `docs/product/demo-role-readiness.md`
- Work order selected: Add an exact filtered Student Directory handoff to recent program-scoped Global Admin role assignments when the grant maps to one accessible school.
- Selection reason: current repo evidence showed a real protected-workspace gap, not speculative polish. `workspace.js` already surfaced recent role assignments with readable scope names, school-scoped handoffs, and assigner labels, but program-scoped grants still stopped at passive copy even though `/api/admin/role-assignments` could safely resolve active school mappings from `site_programs` and the existing Student Directory already supported exact `programId` filters. A single-school mapping gave this run one precise destination without widening scope or inventing a new route.
- Candidate scoring summary:

| Candidate | Ladder Level | Roles | Impact | Safety | Testability | Size | Score | Decision |
|---|---|---|---:|---:|---:|---|---:|---|
| Role-assignment program-student handoff | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | `global_admin` | 5 | 5 | 5 | XS | 60 | selected |
| Role-assignment cohort-student handoff | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | `global_admin` | 4 | 3 | 3 | S | 40 | deferred: still needs exact cohort-to-school resolution before a real action can be shown |
| Broader Global Admin route refresh across Recent Audit, Students, Presentation, Programs, and Users & Access | `LEVEL_9_AUTONOMOUS_QUALITY_IMPROVEMENT` | `global_admin` | 4 | 4 | 3 | M | 44 | deferred: broader than one safe hourly slice |
| Filtered Audit cross-school proof refresh | `LEVEL_7_AUDITABILITY_AND_TRUST` | `global_admin` | 4 | 3 | 3 | M | 36 | deferred: needs a tighter locally provable scenario first |
| Admin mentor-meeting exact destination | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | `global_admin` | 4 | 2 | 2 | M | 31 | deferred: current workspace still lacks a precise meeting-follow-up destination |
| Program Teacher mentor-meeting follow-up destination | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | `program_teacher` | 4 | 3 | 3 | M | 40 | deferred: no precise scoped destination is proven |
...
- User-facing improvement: Global Admin recent role assignments can now open the existing Student Directory filtered to the assigned program when that grant maps to one accessible school, instead of stopping at a passive program label.
- Roles affected: `global_admin`
- Files changed: `functions/api/admin/role-assignments.ts`, `workspace.js`, `tests/admin-role-assignments.integration.test.mjs`, `tests/workspace-app.test.mjs`, `docs/functionality-language-audit.md`, `docs/product/demo-role-readiness.md`, `docs/progress/run-log.md`, `docs/progress/runs/2026-06-03-0111-role-assignment-program-students.json`, `docs/functionality-ux-growth-ledger.md`, `automation/state/functionality-ux-growth-state.json`
- Tests/verifiers added or updated: `tests/admin-role-assignments.integration.test.mjs` now proves `/api/admin/role-assignments` returns mapped school ids for program-scoped grants; `tests/workspace-app.test.mjs` now proves the Global Admin disclosure renders the new program handoff and opens the existing Student Directory with the exact `siteId` plus `programId` filter.
- Validation commands:
  - `node --check workspace.js`
  - `node --test tests/admin-role-assignments.integration.test.mjs`
  - `node --test tests/workspace-app.test.mjs`
  ...
  - `npm run test`
  - `npm run check`
  - `node -e "JSON.parse(require('fs').readFileSync('automation/state/functionality-ux-growth-state.json','utf8')); JSON.parse(require('fs').readFileSync('docs/progress/runs/2026-06-03-0111-role-assignment-program-students.json','utf8')); console.log('json ok')"`
  - `git diff --check`
- Validation result: passed; focused role-assignment route and workspace coverage passed before docs/state closeout, and the full validation ladder passed after the closeout files were updated.
- Commit: pending closeout commit
- Push status: not pushed
- Deferred items: role-assignment cohort drill-down depth, broader Global Admin route refresh, filtered Audit cross-school proof refresh, Admin mentor-meeting exact destination, Program Teacher mentor-meeting follow-up destination, hosted section-level permission proof, Review Queue missing-submission semantics, viewer email visibility policy, and mentor reassignment/remove workflow remain open.
- New backlog items: none
- Next recommended work order: Continue the bounded Global Admin role-assignment refresh only where the current workspace can prove one exact school; inspect cohort-scoped grants next and keep multi-school program grants summary-only until a precise mapped destination exists.
- Do-not-repeat notes: do not auto-open program-scoped recent role assignments when the grant maps to multiple schools; do not remove `scopeSiteIds` from the protected role-assignment route while the workspace uses them to prove exact student-list handoffs; and do not broaden this Global Admin-only disclosure into site-scoped workspaces.
- Ladder Handoff:
  - Targeted Level: `LEVEL_4_ROLE_SPECIFIC_WORKSPACES`
  - Advanced: yes
  - Evidence: `/api/admin/role-assignments` now returns mapped school ids for program-scoped grants, and `renderAdminRoleAssignmentsBody()` can switch directly into the existing filtered Student Directory when one accessible school is proven.
  - Unlocks: the next role-assignment pass can inspect cohort-scoped and multi-school grant follow-ups without rediscovering the program-grant dead end.
  - Next: inspect `workspace.js` around `renderAdminRoleAssignmentsBody()` and `functions/api/admin/role-assignments.ts` for the remaining exact cohort or multi-school handoff gaps only if one precise school can be proven.
  - Blockers: multi-school program grants still need an exact mapped destination contract; mentor-meeting follow-up still lacks a precise destination; hosted permission proof still needs allowed runtime.
  - Do not repeat: do not regress single-school program grants back to passive summary-only rows while the protected route and existing Student Directory already support the exact handoff.
  - First file to inspect next run: `workspace.js` around `renderAdminRoleAssignmentsBody()` and `functions/api/admin/role-assignments.ts`

## Run 2026-06-03 01:43 PT

- Starting SHA: `cb730bfd3e3c173d8c48d827d5f221da786434f8`
- Ending SHA: pending closeout commit; final hash is reported after commit
- Branch: `main`
- Branch policy: started from clean local `main` ahead of `origin/main` by ten earlier in-lane commits; this run stayed local-only and did not push
- Ladder level targeted: `LEVEL_4_ROLE_SPECIFIC_WORKSPACES`
- Backlog item: remaining Global Admin role-assignment handoff depth inside `docs/functionality-language-audit.md` and `docs/product/demo-role-readiness.md`
- Work order selected: Add an exact filtered Student Directory handoff to recent cohort-scoped Global Admin role assignments when the grant maps to one accessible school.
- Selection reason: current repo evidence showed a real protected-workspace gap, not speculative polish. The previous run already proved exact single-school program handoffs, but cohort-scoped grants still stayed summary-only because `/api/admin/role-assignments` did not yet resolve school mappings for cohorts and the workspace only knew how to open program-scoped rows. The existing Student Directory already supported exact `cohortId` filters, so extending the same protected route and disclosure kept the slice route-backed, testable, and RBAC-safe.
- Candidate scoring summary:

| Candidate | Ladder Level | Roles | Impact | Safety | Testability | Size | Score | Decision |
|---|---|---|---:|---:|---:|---|---:|---|
| Role-assignment cohort-student handoff | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | `global_admin` | 5 | 5 | 5 | XS | 60 | selected |
| Multi-school role-assignment destination contract | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | `global_admin` | 4 | 3 | 2 | M | 35 | deferred: exact multi-school destination is still not proven |
| Broader Global Admin route refresh across Recent Audit, Students, Presentation, Programs, and Users & Access | `LEVEL_9_AUTONOMOUS_QUALITY_IMPROVEMENT` | `global_admin` | 4 | 4 | 3 | M | 44 | deferred: broader than one safe hourly slice |
| Filtered Audit cross-school proof refresh | `LEVEL_7_AUDITABILITY_AND_TRUST` | `global_admin` | 4 | 3 | 3 | M | 36 | deferred: needs a tighter locally provable scenario first |
| Admin mentor-meeting exact destination | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | `global_admin` | 4 | 2 | 2 | M | 31 | deferred: current workspace still lacks a precise meeting-follow-up destination |
| Program Teacher mentor-meeting follow-up destination | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | `program_teacher` | 4 | 3 | 3 | M | 40 | deferred: no precise scoped destination is proven |
...
- User-facing improvement: Global Admin recent role assignments can now open the existing Student Directory filtered to the assigned cohort when that grant maps to one accessible school, instead of leaving cohort rows summary-only.
- Roles affected: `global_admin`
- Files changed: `functions/api/admin/role-assignments.ts`, `functions/api/site/students.ts`, `workspace.js`, `tests/admin-role-assignments.integration.test.mjs`, `tests/workspace-app.test.mjs`, `docs/functionality-language-audit.md`, `docs/product/demo-role-readiness.md`, `docs/progress/run-log.md`, `docs/progress/runs/2026-06-03-0143-role-assignment-cohort-students.json`, `docs/functionality-ux-growth-ledger.md`, `automation/state/functionality-ux-growth-state.json`
- Tests/verifiers added or updated: `tests/admin-role-assignments.integration.test.mjs` now proves `/api/admin/role-assignments` returns mapped school ids for cohort-scoped grants, and `tests/workspace-app.test.mjs` now proves the Global Admin disclosure renders the cohort handoff and opens the existing Student Directory with the exact `siteId` plus `cohortId` filter.
- Validation commands:
  - `node --check workspace.js`
  - `node --test tests/admin-role-assignments.integration.test.mjs`
  - `node --test tests/workspace-app.test.mjs`
  ...
  - `npm run test`
  - `npm run check`
  - `node -e "JSON.parse(require('fs').readFileSync('automation/state/functionality-ux-growth-state.json','utf8')); JSON.parse(require('fs').readFileSync('docs/progress/runs/2026-06-03-0143-role-assignment-cohort-students.json','utf8')); console.log('json ok')"`
  - `git diff --check`
- Validation result: passed; focused role-assignment route and workspace coverage passed before docs/state closeout, and the full validation ladder passed after the closeout files were updated.
- Commit: pending closeout commit
- Push status: not pushed
- Deferred items: broader Global Admin route refresh, multi-school role-assignment destination proof, filtered Audit cross-school proof refresh, Admin mentor-meeting exact destination, Program Teacher mentor-meeting follow-up destination, hosted section-level permission proof, Review Queue missing-submission semantics, viewer email visibility policy, and mentor reassignment/remove workflow remain open.
- New backlog items: none
- Next recommended work order: Continue the bounded Global Admin role-assignment refresh only where the current workspace can prove one exact school; inspect remaining multi-school program or cohort grants next and keep those rows summary-only until a precise mapped destination exists.
- Do-not-repeat notes: do not auto-open cohort-scoped or program-scoped recent role assignments when the grant maps to multiple accessible schools; do not remove `scopeSiteIds` from the protected role-assignment route while the workspace uses them to prove exact student-list handoffs; and do not regress the single-school cohort handoff back to a passive role-assignment row while the existing Student Directory already supports the exact `cohortId` filter.
- Ladder Handoff:
  - Targeted Level: `LEVEL_4_ROLE_SPECIFIC_WORKSPACES`
  - Advanced: yes
  - Evidence: `/api/admin/role-assignments` now returns mapped school ids for cohort-scoped grants, and `renderAdminRoleAssignmentsBody()` can switch directly into the existing filtered Student Directory when one accessible school is proven.
  - Unlocks: the next role-assignment pass can inspect remaining multi-school grant follow-ups without rediscovering cohort rows as passive history.
  - Next: inspect `workspace.js` around `renderAdminRoleAssignmentsBody()`, `functions/api/admin/role-assignments.ts`, and the remaining exact multi-school handoff gaps only if one precise mapped destination can be proven.
...
## Run 2026-06-03 05:06 PT

- Starting SHA: `00a4c323c3f0ea914007085fac1d333b1862c87e`
- Ending SHA: pending closeout commit; final hash is reported after commit
- Branch: `main`
- Branch policy: started from clean local `main` ahead of `origin/main` by seventeen earlier in-lane commits; this run stayed local-only and did not push
- Ladder level targeted: `LEVEL_6_STUDENT_PROGRESS_DRILL_DOWN`
- Backlog item: student requirement-detail evidence follow-through inside `docs/functionality-language-audit.md` and `docs/product/demo-role-readiness.md`
- Work order selected: show the matching uploaded and linked work inside student requirement detail
- Selection reason: current repo evidence showed a real follow-through gap, not speculative polish. The previous student slice already let Uploaded and linked work reopen the right requirement, but the requirement detail itself still stopped at summary facts and latest feedback. Students could reach the right requirement and still had to bounce back to the files list to confirm what was attached. Reusing the current student-scoped evidence rows inside the same detail panel kept the slice small, exact, and route-free.
- Candidate scoring summary:

| Candidate | Ladder Level | Roles | Impact | Safety | Testability | Size | Score | Decision |
|---|---|---|---:|---:|---:|---|---:|---|
| Student requirement-detail evidence follow-through | `LEVEL_6_STUDENT_PROGRESS_DRILL_DOWN` | `student` | 5 | 5 | 5 | XS | 58 | selected |
| Filtered Audit cross-school proof refresh | `LEVEL_7_AUDITABILITY_AND_TRUST` | `global_admin` | 4 | 3 | 3 | M | 37 | deferred: still needs one tighter locally provable scenario |
| Student next-step anchor follow-through | `LEVEL_6_STUDENT_PROGRESS_DRILL_DOWN` | `student` | 4 | 4 | 3 | S | 41 | deferred: broader than this detail-panel batch |
| Student feedback timeline handoff from requirement detail | `LEVEL_6_STUDENT_PROGRESS_DRILL_DOWN` | `student` | 4 | 4 | 3 | S | 41 | deferred: keep for a later exact same-panel handoff |
| Student evidence-panel language cleanup | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | `student` | 2 | 5 | 4 | XS | 34 | deferred: lower value than the selected drill-down gap |
| Empty load versus no-data follow-through | `LEVEL_4_ROLE_SPECIFIC_WORKSPACES` | staff and student | 3 | 4 | 3 | S | 35 | deferred: broader shared-state pass than this slice |
| Broader Global Admin route refresh across Audit, Programs, Users & Access, and Presentation | `LEVEL_9_AUTONOMOUS_QUALITY_IMPROVEMENT` | `global_admin` | 4 | 4 | 3 | M | 43 | deferred: broader than one safe hourly slice |
...
- User-facing improvement: students who reopen a requirement from Uploaded and linked work can now see the exact attached files or links inside that same requirement detail and keep the existing safe file actions.
- Roles affected: `student`
- Files changed: `workspace.js`, `tests/workspace-app.test.mjs`, `tests/functionality-language-audit.test.mjs`, `docs/functionality-language-audit.md`, `docs/product/demo-role-readiness.md`, `docs/progress/run-log.md`, `docs/progress/runs/2026-06-03-0506-student-requirement-detail-evidence.json`, `docs/functionality-ux-growth-ledger.md`, `automation/state/functionality-ux-growth-state.json`
- Tests/verifiers added or updated: `tests/workspace-app.test.mjs` now proves both direct requirement-detail opens and file-row reopens render the matching attached item inside `Matching uploaded and linked work`; `tests/functionality-language-audit.test.mjs` now tracks the updated audit summary count after the new completed slice.
- Validation commands:
  - `node --check workspace.js`
  - `node --test tests/workspace-app.test.mjs`
  - `npm run verify:functionality-language`
  ...
  - `npm run test`
  - `npm run check`
  - `node -e "JSON.parse(require('fs').readFileSync('automation/state/functionality-ux-growth-state.json','utf8')); JSON.parse(require('fs').readFileSync('docs/progress/runs/2026-06-03-0506-student-requirement-detail-evidence.json','utf8')); console.log('json ok')"`
  - `git diff --check`
...
- Commit: pending closeout commit
- Push status: not pushed
- Deferred items: filtered Audit cross-school proof refresh, student next-step anchor follow-through, requirement-detail feedback-timeline follow-through, student evidence-panel language cleanup, broader Global Admin route refresh, site-selector follow-through, mentor reassignment/remove workflow, and hosted section-level permission proof remain open.
- New backlog items: none
- Next recommended work order: continue the student refresh only if the current requirement detail or submission panels can prove another exact feedback or submission timeline handoff; otherwise return to filtered Audit cross-school proof only if a tighter local scenario is now provable
- Do-not-repeat notes: do not regress student requirement detail back to summary-only facts while the current dashboard already carries the matching evidence rows; do not remove the safe file actions from requirement-detail evidence rows while Uploaded and linked work already exposes the same student-scoped actions; and do not expose storage identifiers, new evidence routes, or other-student file context while deepening student detail.
- Ladder Handoff:
  - Targeted Level: `LEVEL_6_STUDENT_PROGRESS_DRILL_DOWN`
  - Advanced: yes
  - Evidence: requirement detail now renders `Matching uploaded and linked work`, focused workspace tests prove both direct requirement opens and file-row reopens show the attached item, and the same student-scoped file actions stay intact without adding a route.
  - Unlocks: the next student-facing pass can deepen feedback or submission history from the same requirement-detail context instead of bouncing students back to separate panels.
  - Next: inspect `workspace.js` student requirement detail and existing student feedback/submission timeline actions only if another same-panel handoff is exact and already backed by current route data; otherwise move back to filtered Audit cross-school proof when a tighter local scenario exists.
  - Blockers: filtered Audit cross-school proof still needs a tighter local scenario; hosted permission proof still needs allowed runtime; mentor reassignment/remove still needs mutation and audit design.
