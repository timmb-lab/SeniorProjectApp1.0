...
For 30-day auto-scaling audits, run `scripts/measure-automation-efficiency.ps1` non-interactively and use the output to decide whether the system should keep cadence, retarget QoL focus, reduce collision risk, sharpen blockers, or ask Bryan for a schedule change. Do not increase starts merely because capacity exists; the current system needs only 6.67 percent accepted-pass conversion to hit the 2/day minimum over 30 days.
