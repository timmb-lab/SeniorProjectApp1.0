...
- Program Teacher demo proof intentionally leaves a few primary IT Missing Mentor Demo students without attached evidence, so the Missing Evidence dashboard card has a visible selected-site Student Directory click-through.
- Phase 12 `/api/site/operations-readiness` proof uses Presentation Pending Demo, Archive Ready Demo, Archive Failed Demo, High Risk Demo, and Rich Timeline Demo rows for selected-site end-of-project worklists. The route is read-only, bounded, redacted, and lets worklist rows open the existing Phase 9 student detail drawer.
