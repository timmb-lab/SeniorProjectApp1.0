...
| Mentor | Local proof keeps Mentor denied from `/api/site/programs` while preserving assigned-student mentor dashboard paths. Mentor Dashboard rows now render as compact assigned-student summaries with secondary meeting/presentation/activity details disclosed per row, and mentor-opened student detail can now reload or share from the current mentor-dashboard focus URL without broadening access. | Needs hosted/live proof and a broader mentor-support workflow review. |
...
- Mentor: locally proven denied from `/api/site/programs`; assigned-student mentor routes remain covered by prior local proof, and mentor-opened student detail now restores from the same mentor-dashboard focus URL.
