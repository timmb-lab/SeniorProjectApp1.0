...
test("project demo reset defines exactly ten varied, fully staffed projects", () => {
  ...
  assert.deepEqual([...new Set(PROJECTS.map((row) => row.students.length))].sort(), [1, 2, 3, 4, 5]);
...
test("project demo reset keeps identity, school, program, template, and setting tables", () => {
  ...
  assert.equal((sql.match(/INSERT INTO projects \(/g) || []).length, 10);
  assert.equal((sql.match(/INSERT INTO project_adult_assignments/g) || []).length, 20);
