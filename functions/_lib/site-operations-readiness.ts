...
function canUseOperationsRole(roleIds: RoleId[]): boolean {
  return roleIds.some((roleId) => ["platform_admin", "admin", "org_admin", "site_admin", "viewer", "program_teacher"].includes(roleId));
}

function hasAnyAdminLikeRole(roleIds: RoleId[]): boolean {
  return roleIds.some((roleId) => ["platform_admin", "admin", "org_admin", "site_admin", "viewer"].includes(roleId));
}
